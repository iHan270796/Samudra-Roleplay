local L0_1, L1_1, L2_1, L3_1, L4_1, L5_1, L6_1, L7_1, L8_1, L9_1, L10_1, L11_1, L12_1, L13_1, L14_1, L15_1
L0_1 = IsDuplicityVersion
L0_1 = L0_1()
if L0_1 then
  L0_1 = GetPlayerPositionInRealTime67
  L0_1()
end
L0_1 = {}
L0_1.seatcategoryid = nil
L0_1.seatid = nil
L1_1 = nil
L2_1 = nil
L3_1 = false
L4_1 = false
L5_1 = 0
L6_1 = {}
L7_1 = vector3
L8_1 = -1637.834
L9_1 = -1078.906
L10_1 = 41.26547
L7_1 = L7_1(L8_1, L9_1, L10_1)
L6_1.coords = L7_1
L6_1.mainhandler = nil
L6_1.animhandler = nil
L6_1.started = false
L6_1.changingsides = false
L6_1.currentrotation = 0.0
L6_1.seatdown = 1
L6_1.stageinprogress = false
L6_1.stage = 0
L6_1.stagecounter = 0
L6_1.stagespeed = 0.5
L6_1.getnew = false
L6_1.synchronizestop = false
L7_1 = {}
L8_1 = {}
L8_1.seathandlermain = nil
L8_1.animhandler = nil
L9_1 = {}
L10_1 = {}
L10_1.handler = nil
L9_1[1] = L10_1
L10_1 = {}
L10_1.handler = nil
L9_1[2] = L10_1
L8_1.seathandlercage = L9_1
L8_1.cageclosed = false
L9_1 = {}
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = 0.4
L14_1 = -0.34
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = -90.0
L10_1.offsets = L11_1
L9_1[1] = L10_1
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = 0.4
L14_1 = -0.98
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = -90.0
L10_1.offsets = L11_1
L9_1[2] = L10_1
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = -0.4
L14_1 = -0.34
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = 90.0
L10_1.offsets = L11_1
L9_1[3] = L10_1
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = -0.4
L14_1 = -0.98
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = 90.0
L10_1.offsets = L11_1
L9_1[4] = L10_1
L8_1.seats = L9_1
L7_1[1] = L8_1
L8_1 = {}
L8_1.seathandlermain = nil
L8_1.animhandler = nil
L9_1 = {}
L10_1 = {}
L10_1.handler = nil
L9_1[1] = L10_1
L10_1 = {}
L10_1.handler = nil
L9_1[2] = L10_1
L8_1.seathandlercage = L9_1
L8_1.cageclosed = true
L9_1 = {}
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = 0.4
L14_1 = -0.34
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = -90.0
L10_1.offsets = L11_1
L9_1[1] = L10_1
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = 0.4
L14_1 = -0.98
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = -90.0
L10_1.offsets = L11_1
L9_1[2] = L10_1
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = -0.4
L14_1 = -0.34
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = 90.0
L10_1.offsets = L11_1
L9_1[3] = L10_1
L10_1 = {}
L10_1.taken = false
L10_1.takenplayerid = nil
L11_1 = {}
L12_1 = vector3
L13_1 = -0.4
L14_1 = -0.98
L15_1 = -0.15
L12_1 = L12_1(L13_1, L14_1, L15_1)
L11_1.coords = L12_1
L11_1.heading = 90.0
L10_1.offsets = L11_1
L9_1[4] = L10_1
L8_1.seats = L9_1
L7_1[2] = L8_1
L6_1.seats = L7_1
gforcehandler = L6_1
L6_1 = RegisterNetEvent
L7_1 = "rtx_themepark:GForce:SynchronizeMovement"
L6_1(L7_1)
L6_1 = AddEventHandler
L7_1 = "rtx_themepark:GForce:SynchronizeMovement"
function L8_1(A0_2, A1_2, A2_2)
  local L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2
  L3_2 = nearbythemepark
  if true == L3_2 then
    if 13 == A2_2 or 15 == A2_2 or 20 == A2_2 then
      L3_2 = gforcehandler
      L3_2.getnew = true
      L3_2 = gforcehandler
      L3_2.synchronizestop = false
      L3_2 = gforcehandler
      L3_2.currentrotation = A0_2
      L3_2 = SetEntityRotation
      L4_2 = gforcehandler
      L4_2 = L4_2.mainhandler
      L5_2 = 0.0
      L6_2 = A0_2
      L7_2 = 230.0
      L8_2 = false
      L3_2(L4_2, L5_2, L6_2, L7_2, L8_2)
      L3_2 = ipairs
      L4_2 = gforcehandler
      L4_2 = L4_2.seats
      L3_2, L4_2, L5_2, L6_2 = L3_2(L4_2)
      for L7_2, L8_2 in L3_2, L4_2, L5_2, L6_2 do
        if 1 == L7_2 then
          L9_2 = AttachEntityToEntity
          L10_2 = L8_2.seathandlermain
          L11_2 = gforcehandler
          L11_2 = L11_2.mainhandler
          L12_2 = 0
          L13_2 = 0.0
          L14_2 = 0.4
          L15_2 = -26.4
          L16_2 = 0.0
          L17_2 = A0_2
          L18_2 = 0.0
          L19_2 = false
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = 2
          L24_2 = true
          L9_2(L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2)
        else
          L9_2 = AttachEntityToEntity
          L10_2 = L8_2.seathandlermain
          L11_2 = gforcehandler
          L11_2 = L11_2.mainhandler
          L12_2 = 0
          L13_2 = 0.0
          L14_2 = 0.4
          L15_2 = 26.2
          L16_2 = 0.0
          L17_2 = A0_2
          L18_2 = 0.0
          L19_2 = false
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = 2
          L24_2 = true
          L9_2(L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2)
        end
      end
    else
      L3_2 = gforcehandler
      L3_2.getnew = true
      L3_2 = SetEntityRotation
      L4_2 = gforcehandler
      L4_2 = L4_2.mainhandler
      L5_2 = 0.0
      L6_2 = A0_2
      L7_2 = 230.0
      L8_2 = false
      L3_2(L4_2, L5_2, L6_2, L7_2, L8_2)
      L3_2 = ipairs
      L4_2 = gforcehandler
      L4_2 = L4_2.seats
      L3_2, L4_2, L5_2, L6_2 = L3_2(L4_2)
      for L7_2, L8_2 in L3_2, L4_2, L5_2, L6_2 do
        if 1 == L7_2 then
          L9_2 = AttachEntityToEntity
          L10_2 = L8_2.seathandlermain
          L11_2 = gforcehandler
          L11_2 = L11_2.mainhandler
          L12_2 = 0
          L13_2 = 0.0
          L14_2 = 0.4
          L15_2 = -26.4
          L16_2 = 0.0
          L17_2 = A0_2
          L18_2 = 0.0
          L19_2 = false
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = 2
          L24_2 = true
          L9_2(L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2)
        else
          L9_2 = AttachEntityToEntity
          L10_2 = L8_2.seathandlermain
          L11_2 = gforcehandler
          L11_2 = L11_2.mainhandler
          L12_2 = 0
          L13_2 = 0.0
          L14_2 = 0.4
          L15_2 = 26.2
          L16_2 = 0.0
          L17_2 = A0_2
          L18_2 = 0.0
          L19_2 = false
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = 2
          L24_2 = true
          L9_2(L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2)
        end
      end
      L3_2 = gforcehandler
      L3_2.currentrotation = A0_2
      L3_2 = Citizen
      L3_2 = L3_2.Wait
      L4_2 = 1
      L3_2(L4_2)
      L3_2 = gforcehandler
      L3_2.getnew = false
      L3_2 = gforcehandler
      L3_2.synchronizestop = false
      L3_2 = A1_2 * 0.1
      L4_2 = 1
      L5_2 = currentfps
      if L5_2 < 70 then
        L3_2 = A1_2 * 0.25
        L4_2 = 0
      else
        L5_2 = currentfps
        if L5_2 < 110 then
          L3_2 = A1_2 * 0.15
          L4_2 = 0
        end
      end
      while true do
        L5_2 = gforcehandler
        L5_2 = L5_2.getnew
        if false ~= L5_2 then
          break
        end
        L5_2 = gforcehandler
        L5_2 = L5_2.synchronizestop
        if false ~= L5_2 then
          break
        end
        L5_2 = nearbythemepark
        if true ~= L5_2 then
          break
        end
        L5_2 = Citizen
        L5_2 = L5_2.Wait
        L6_2 = L4_2
        L5_2(L6_2)
        if 12 == A2_2 then
          L5_2 = gforcehandler
          L5_2 = L5_2.currentrotation
          L6_2 = 179.9
          if not (L5_2 < L6_2) then
            L5_2 = gforcehandler
            L5_2 = L5_2.currentrotation
            L6_2 = 180.1
            if L5_2 > L6_2 then
            else
              L5_2 = gforcehandler
              L5_2.currentrotation = 180.0
              L5_2 = SetEntityRotation
              L6_2 = gforcehandler
              L6_2 = L6_2.mainhandler
              L7_2 = 0.0
              L8_2 = gforcehandler
              L8_2 = L8_2.currentrotation
              L9_2 = 230.0
              L10_2 = false
              L5_2(L6_2, L7_2, L8_2, L9_2, L10_2)
              L5_2 = ipairs
              L6_2 = gforcehandler
              L6_2 = L6_2.seats
              L5_2, L6_2, L7_2, L8_2 = L5_2(L6_2)
              for L9_2, L10_2 in L5_2, L6_2, L7_2, L8_2 do
                if 1 == L9_2 then
                  L11_2 = AttachEntityToEntity
                  L12_2 = L10_2.seathandlermain
                  L13_2 = gforcehandler
                  L13_2 = L13_2.mainhandler
                  L14_2 = 0
                  L15_2 = 0.0
                  L16_2 = 0.4
                  L17_2 = -26.4
                  L18_2 = 0.0
                  L19_2 = gforcehandler
                  L19_2 = L19_2.currentrotation
                  L20_2 = 0.0
                  L21_2 = false
                  L22_2 = false
                  L23_2 = false
                  L24_2 = false
                  L25_2 = 2
                  L26_2 = true
                  L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
                else
                  L11_2 = AttachEntityToEntity
                  L12_2 = L10_2.seathandlermain
                  L13_2 = gforcehandler
                  L13_2 = L13_2.mainhandler
                  L14_2 = 0
                  L15_2 = 0.0
                  L16_2 = 0.4
                  L17_2 = 26.2
                  L18_2 = 0.0
                  L19_2 = gforcehandler
                  L19_2 = L19_2.currentrotation
                  L20_2 = 0.0
                  L21_2 = false
                  L22_2 = false
                  L23_2 = false
                  L24_2 = false
                  L25_2 = 2
                  L26_2 = true
                  L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
                end
              end
              L5_2 = gforcehandler
              L5_2.synchronizestop = true
            end
          end
        elseif 22 == A2_2 then
          L5_2 = gforcehandler
          L5_2 = L5_2.currentrotation
          L6_2 = 179.9
          if not (L5_2 < L6_2) then
            L5_2 = gforcehandler
            L5_2 = L5_2.currentrotation
            L6_2 = 180.1
            if L5_2 > L6_2 then
            else
              L5_2 = gforcehandler
              L5_2.currentrotation = 180.0
              L5_2 = SetEntityRotation
              L6_2 = gforcehandler
              L6_2 = L6_2.mainhandler
              L7_2 = 0.0
              L8_2 = gforcehandler
              L8_2 = L8_2.currentrotation
              L9_2 = 230.0
              L10_2 = false
              L5_2(L6_2, L7_2, L8_2, L9_2, L10_2)
              L5_2 = ipairs
              L6_2 = gforcehandler
              L6_2 = L6_2.seats
              L5_2, L6_2, L7_2, L8_2 = L5_2(L6_2)
              for L9_2, L10_2 in L5_2, L6_2, L7_2, L8_2 do
                if 1 == L9_2 then
                  L11_2 = AttachEntityToEntity
                  L12_2 = L10_2.seathandlermain
                  L13_2 = gforcehandler
                  L13_2 = L13_2.mainhandler
                  L14_2 = 0
                  L15_2 = 0.0
                  L16_2 = 0.4
                  L17_2 = -26.4
                  L18_2 = 0.0
                  L19_2 = gforcehandler
                  L19_2 = L19_2.currentrotation
                  L20_2 = 0.0
                  L21_2 = false
                  L22_2 = false
                  L23_2 = false
                  L24_2 = false
                  L25_2 = 2
                  L26_2 = true
                  L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
                else
                  L11_2 = AttachEntityToEntity
                  L12_2 = L10_2.seathandlermain
                  L13_2 = gforcehandler
                  L13_2 = L13_2.mainhandler
                  L14_2 = 0
                  L15_2 = 0.0
                  L16_2 = 0.4
                  L17_2 = 26.2
                  L18_2 = 0.0
                  L19_2 = gforcehandler
                  L19_2 = L19_2.currentrotation
                  L20_2 = 0.0
                  L21_2 = false
                  L22_2 = false
                  L23_2 = false
                  L24_2 = false
                  L25_2 = 2
                  L26_2 = true
                  L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
                end
              end
              L5_2 = gforcehandler
              L5_2.synchronizestop = true
            end
          end
        elseif 14 == A2_2 then
          L5_2 = gforcehandler
          L5_2 = L5_2.currentrotation
          L6_2 = 359.9
          if L5_2 < L6_2 then
          else
            L5_2 = gforcehandler
            L5_2.currentrotation = 360.0
            L5_2 = SetEntityRotation
            L6_2 = gforcehandler
            L6_2 = L6_2.mainhandler
            L7_2 = 0.0
            L8_2 = gforcehandler
            L8_2 = L8_2.currentrotation
            L9_2 = 230.0
            L10_2 = false
            L5_2(L6_2, L7_2, L8_2, L9_2, L10_2)
            L5_2 = ipairs
            L6_2 = gforcehandler
            L6_2 = L6_2.seats
            L5_2, L6_2, L7_2, L8_2 = L5_2(L6_2)
            for L9_2, L10_2 in L5_2, L6_2, L7_2, L8_2 do
              if 1 == L9_2 then
                L11_2 = AttachEntityToEntity
                L12_2 = L10_2.seathandlermain
                L13_2 = gforcehandler
                L13_2 = L13_2.mainhandler
                L14_2 = 0
                L15_2 = 0.0
                L16_2 = 0.4
                L17_2 = -26.4
                L18_2 = 0.0
                L19_2 = gforcehandler
                L19_2 = L19_2.currentrotation
                L20_2 = 0.0
                L21_2 = false
                L22_2 = false
                L23_2 = false
                L24_2 = false
                L25_2 = 2
                L26_2 = true
                L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
              else
                L11_2 = AttachEntityToEntity
                L12_2 = L10_2.seathandlermain
                L13_2 = gforcehandler
                L13_2 = L13_2.mainhandler
                L14_2 = 0
                L15_2 = 0.0
                L16_2 = 0.4
                L17_2 = 26.2
                L18_2 = 0.0
                L19_2 = gforcehandler
                L19_2 = L19_2.currentrotation
                L20_2 = 0.0
                L21_2 = false
                L22_2 = false
                L23_2 = false
                L24_2 = false
                L25_2 = 2
                L26_2 = true
                L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
              end
            end
            L5_2 = gforcehandler
            L5_2.synchronizestop = true
          end
        else
          L5_2 = gforcehandler
          L5_2 = L5_2.currentrotation
          L6_2 = 360.0
          if L5_2 > L6_2 then
            L5_2 = gforcehandler
            L5_2 = L5_2.currentrotation
            L5_2 = L5_2 - 360.0
            L6_2 = gforcehandler
            L7_2 = 0.0 + L5_2
            L6_2.currentrotation = L7_2
          end
        end
        L5_2 = gforcehandler
        L5_2 = L5_2.synchronizestop
        if false == L5_2 then
          L5_2 = gforcehandler
          L6_2 = gforcehandler
          L6_2 = L6_2.currentrotation
          L6_2 = L6_2 + L3_2
          L5_2.currentrotation = L6_2
        end
        L5_2 = gforcehandler
        L5_2 = L5_2.getnew
        if false == L5_2 then
          L5_2 = SetEntityRotation
          L6_2 = gforcehandler
          L6_2 = L6_2.mainhandler
          L7_2 = 0.0
          L8_2 = gforcehandler
          L8_2 = L8_2.currentrotation
          L9_2 = 230.0
          L10_2 = false
          L5_2(L6_2, L7_2, L8_2, L9_2, L10_2)
          L5_2 = ipairs
          L6_2 = gforcehandler
          L6_2 = L6_2.seats
          L5_2, L6_2, L7_2, L8_2 = L5_2(L6_2)
          for L9_2, L10_2 in L5_2, L6_2, L7_2, L8_2 do
            if 1 == L9_2 then
              L11_2 = AttachEntityToEntity
              L12_2 = L10_2.seathandlermain
              L13_2 = gforcehandler
              L13_2 = L13_2.mainhandler
              L14_2 = 0
              L15_2 = 0.0
              L16_2 = 0.4
              L17_2 = -26.4
              L18_2 = 0.0
              L19_2 = gforcehandler
              L19_2 = L19_2.currentrotation
              L20_2 = 0.0
              L21_2 = false
              L22_2 = false
              L23_2 = false
              L24_2 = false
              L25_2 = 2
              L26_2 = true
              L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
            else
              L11_2 = AttachEntityToEntity
              L12_2 = L10_2.seathandlermain
              L13_2 = gforcehandler
              L13_2 = L13_2.mainhandler
              L14_2 = 0
              L15_2 = 0.0
              L16_2 = 0.4
              L17_2 = 26.2
              L18_2 = 0.0
              L19_2 = gforcehandler
              L19_2 = L19_2.currentrotation
              L20_2 = 0.0
              L21_2 = false
              L22_2 = false
              L23_2 = false
              L24_2 = false
              L25_2 = 2
              L26_2 = true
              L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2)
            end
          end
        end
      end
    end
  else
    L3_2 = gforcehandler
    L3_2.getnew = true
  end
end
L6_1(L7_1, L8_1)
L6_1 = RegisterNetEvent
L7_1 = "rtx_themepark:GForce:SynchronizeStarted"
L6_1(L7_1)
L6_1 = AddEventHandler
L7_1 = "rtx_themepark:GForce:SynchronizeStarted"
function L8_1(A0_2)
  local L1_2, L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2
  L1_2 = gforcehandler
  L1_2.started = A0_2
  L1_2 = DoesEntityExist
  L2_2 = gforcehandler
  L2_2 = L2_2.mainhandler
  L1_2 = L1_2(L2_2)
  if L1_2 then
    if true == A0_2 then
      L1_2 = SetEntityCompletelyDisableCollision
      L2_2 = gforcehandler
      L2_2 = L2_2.mainhandler
      L3_2 = false
      L4_2 = false
      L1_2(L2_2, L3_2, L4_2)
    else
      L1_2 = SetEntityCompletelyDisableCollision
      L2_2 = gforcehandler
      L2_2 = L2_2.mainhandler
      L3_2 = true
      L4_2 = true
      L1_2(L2_2, L3_2, L4_2)
      L1_2 = ipairs
      L2_2 = gforcehandler
      L2_2 = L2_2.seats
      L1_2, L2_2, L3_2, L4_2 = L1_2(L2_2)
      for L5_2, L6_2 in L1_2, L2_2, L3_2, L4_2 do
        if 1 == L5_2 then
          L7_2 = AttachEntityToEntity
          L8_2 = L6_2.seathandlermain
          L9_2 = gforcehandler
          L9_2 = L9_2.mainhandler
          L10_2 = 0
          L11_2 = 0.0
          L12_2 = 0.4
          L13_2 = -26.4
          L14_2 = 0.0
          L15_2 = gforcehandler
          L15_2 = L15_2.currentrotation
          L16_2 = 0.0
          L17_2 = false
          L18_2 = false
          L19_2 = true
          L20_2 = false
          L21_2 = 2
          L22_2 = true
          L7_2(L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2)
        else
          L7_2 = AttachEntityToEntity
          L8_2 = L6_2.seathandlermain
          L9_2 = gforcehandler
          L9_2 = L9_2.mainhandler
          L10_2 = 0
          L11_2 = 0.0
          L12_2 = 0.4
          L13_2 = 26.2
          L14_2 = 0.0
          L15_2 = gforcehandler
          L15_2 = L15_2.currentrotation
          L16_2 = 0.0
          L17_2 = false
          L18_2 = false
          L19_2 = true
          L20_2 = false
          L21_2 = 2
          L22_2 = true
          L7_2(L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2)
        end
      end
    end
  end
end
L6_1(L7_1, L8_1)
L6_1 = RegisterNetEvent
L7_1 = "rtx_themepark:GForce:SeatDown"
L6_1(L7_1)
L6_1 = AddEventHandler
L7_1 = "rtx_themepark:GForce:SeatDown"
function L8_1(A0_2)
  local L1_2
  L1_2 = gforcehandler
  L1_2.seatdown = A0_2
end
L6_1(L7_1, L8_1)
L6_1 = RegisterNetEvent
L7_1 = "rtx_themepark:GForce:SeatData"
L6_1(L7_1)
L6_1 = AddEventHandler
L7_1 = "rtx_themepark:GForce:SeatData"
function L8_1(A0_2, A1_2)
  local L2_2, L3_2
  L2_2 = SendNUIMessage
  L3_2 = {}
  L3_2.message = "attractionhow"
  L3_2.attractionanimchange = true
  L2_2(L3_2)
  L1_1 = A0_2
  L2_1 = A1_2
  L2_2 = true
  L4_1 = L2_2
  L2_2 = Config
  L2_2 = L2_2.ThemeParkFallSettings
  L2_2 = L2_2.fallchancecheck
  L5_1 = L2_2
end
L6_1(L7_1, L8_1)
L6_1 = RegisterNetEvent
L7_1 = "rtx_themepark:GForce:SeatExit"
L6_1(L7_1)
L6_1 = AddEventHandler
L7_1 = "rtx_themepark:GForce:SeatExit"
function L8_1(A0_2)
  local L1_2, L2_2, L3_2, L4_2, L5_2, L6_2
  L1_2 = gforcehandler
  L1_2 = L1_2.started
  if false == L1_2 then
    L1_2 = gforcehandler
    L1_2 = L1_2.seatdown
    L2_2 = L1_1
    if L1_2 == L2_2 then
      L1_2 = PlayerPedId
      L1_2 = L1_2()
      L2_2 = DetachEntity
      L3_2 = L1_2
      L2_2(L3_2)
      L2_2 = FreezeEntityPosition
      L3_2 = L1_2
      L4_2 = false
      L2_2(L3_2, L4_2)
      L2_2 = ClearPedTasks
      L3_2 = L1_2
      L2_2(L3_2)
      L2_2 = SendNUIMessage
      L3_2 = {}
      L3_2.message = "hideattraction"
      L2_2(L3_2)
      L2_2 = nil
      L1_1 = L2_2
      L2_2 = nil
      L2_1 = L2_2
  end
  elseif true == A0_2 then
    L1_2 = PlayerPedId
    L1_2 = L1_2()
    L2_2 = DetachEntity
    L3_2 = L1_2
    L2_2(L3_2)
    L2_2 = FreezeEntityPosition
    L3_2 = L1_2
    L4_2 = false
    L2_2(L3_2, L4_2)
    L2_2 = ClearPedTasks
    L3_2 = L1_2
    L2_2(L3_2)
    L2_2 = SendNUIMessage
    L3_2 = {}
    L3_2.message = "hideattraction"
    L2_2(L3_2)
    L2_2 = nil
    L1_1 = L2_2
    L2_2 = nil
    L2_1 = L2_2
  else
    L1_2 = PlayerPedId
    L1_2 = L1_2()
    L2_2 = DetachEntity
    L3_2 = L1_2
    L2_2(L3_2)
    L2_2 = FreezeEntityPosition
    L3_2 = L1_2
    L4_2 = false
    L2_2(L3_2, L4_2)
    L2_2 = ClearPedTasks
    L3_2 = L1_2
    L2_2(L3_2)
    L2_2 = SetEntityCoordsNoOffset
    L3_2 = L1_2
    L4_2 = Config
    L4_2 = L4_2.AttractionsSettings
    L4_2 = L4_2.gforce
    L4_2 = L4_2.exitcoords
    L4_2 = L4_2.coords
    L4_2 = L4_2.x
    L5_2 = Config
    L5_2 = L5_2.AttractionsSettings
    L5_2 = L5_2.gforce
    L5_2 = L5_2.exitcoords
    L5_2 = L5_2.coords
    L5_2 = L5_2.y
    L6_2 = Config
    L6_2 = L6_2.AttractionsSettings
    L6_2 = L6_2.gforce
    L6_2 = L6_2.exitcoords
    L6_2 = L6_2.coords
    L6_2 = L6_2.z
    L2_2(L3_2, L4_2, L5_2, L6_2)
    L2_2 = SetEntityHeading
    L3_2 = L1_2
    L4_2 = Config
    L4_2 = L4_2.AttractionsSettings
    L4_2 = L4_2.gforce
    L4_2 = L4_2.exitcoords
    L4_2 = L4_2.heading
    L2_2(L3_2, L4_2)
    L2_2 = SendNUIMessage
    L3_2 = {}
    L3_2.message = "hideattraction"
    L2_2(L3_2)
    L2_2 = nil
    L1_1 = L2_2
    L2_2 = nil
    L2_1 = L2_2
  end
  L1_2 = false
  L4_1 = L1_2
end
L6_1(L7_1, L8_1)
L6_1 = Config
L6_1 = L6_1.ThemeParkAttractionFallChance
if L6_1 then
  L6_1 = Config
  L6_1 = L6_1.ThemeParkFallSettings
  L6_1 = L6_1.attractions
  L6_1 = L6_1.gforce
  if L6_1 then
    L6_1 = RegisterNetEvent
    L7_1 = "rtx_themepark:GForce:SeatThrowClient"
    L6_1(L7_1)
    L6_1 = AddEventHandler
    L7_1 = "rtx_themepark:GForce:SeatThrowClient"
    function L8_1()
      local L0_2, L1_2, L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2
      L0_2 = L1_1
      if nil ~= L0_2 then
        L0_2 = L2_1
        if nil ~= L0_2 then
          L0_2 = PlayerPedId
          L0_2 = L0_2()
          L1_2 = DetachEntity
          L2_2 = L0_2
          L1_2(L2_2)
          L1_2 = FreezeEntityPosition
          L2_2 = L0_2
          L3_2 = false
          L1_2(L2_2, L3_2)
          L1_2 = ClearPedTasks
          L2_2 = L0_2
          L1_2(L2_2)
          L1_2 = SendNUIMessage
          L2_2 = {}
          L2_2.message = "hideattraction"
          L1_2(L2_2)
          L1_2 = nil
          L1_1 = L1_2
          L1_2 = nil
          L2_1 = L1_2
          usingattraction = false
          L1_2 = GetEntityForwardVector
          L2_2 = L0_2
          L1_2 = L1_2(L2_2)
          L2_2 = math
          L2_2 = L2_2.random
          L2_2 = L2_2()
          L3_2 = 40.0
          L3_2 = L3_2 - 40.0
          L2_2 = L2_2 * L3_2
          L2_2 = 40.0 + L2_2
          L3_2 = SetPedToRagdoll
          L4_2 = L0_2
          L5_2 = 2000
          L6_2 = 2000
          L7_2 = false
          L8_2 = false
          L9_2 = false
          L10_2 = false
          L3_2(L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2)
          L3_2 = ApplyForceToEntity
          L4_2 = L0_2
          L5_2 = 1
          L6_2 = L1_2.x
          L6_2 = L6_2 * L2_2
          L7_2 = L1_2.y
          L7_2 = L7_2 * L2_2
          L8_2 = L1_2.z
          L8_2 = L8_2 + 40.0
          L9_2 = 0
          L10_2 = 0
          L11_2 = 0
          L12_2 = 0
          L13_2 = false
          L14_2 = true
          L15_2 = true
          L16_2 = false
          L17_2 = true
          L3_2(L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2)
          L3_2 = Notify
          L4_2 = Language
          L5_2 = Config
          L5_2 = L5_2.Language
          L4_2 = L4_2[L5_2]
          L4_2 = L4_2.themeparkfall
          L3_2(L4_2)
        end
      end
    end
    L6_1(L7_1, L8_1)
  end
end
L6_1 = Config
L6_1 = L6_1.Target
if true == L6_1 then
  L6_1 = RegisterNetEvent
  L7_1 = "rtx_themepark:GForce:SeatTarget"
  L6_1(L7_1)
  L6_1 = AddEventHandler
  L7_1 = "rtx_themepark:GForce:SeatTarget"
  function L8_1()
    local L0_2, L1_2, L2_2, L3_2
    L0_2 = gforcehandler
    L0_2 = L0_2.started
    if false == L0_2 then
      L0_2 = gforcehandler
      L0_2 = L0_2.changingsides
      if false == L0_2 then
        L0_2 = usingattraction
        if false == L0_2 then
          L0_2 = L0_1.seatcategoryid
          if nil ~= L0_2 then
            L0_2 = L0_1.seatid
            if nil ~= L0_2 then
              L0_2 = iteminhand
              if false == L0_2 then
                L0_2 = TriggerServerEvent
                L1_2 = "rtx_themepark:GForce:SeatUse"
                L2_2 = L0_1.seatcategoryid
                L3_2 = L0_1.seatid
                L0_2(L1_2, L2_2, L3_2)
              else
                L0_2 = Notify
                L1_2 = Language
                L2_2 = Config
                L2_2 = L2_2.Language
                L1_2 = L1_2[L2_2]
                L1_2 = L1_2.iteminhand
                L0_2(L1_2)
              end
            end
          end
        end
      end
    end
  end
  L6_1(L7_1, L8_1)
end
L6_1 = Citizen
L6_1 = L6_1.CreateThread
function L7_1()
  local L0_2, L1_2, L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2, L27_2, L28_2, L29_2
  while true do
    L0_2 = Citizen
    L0_2 = L0_2.Wait
    L1_2 = 1000
    L0_2(L1_2)
    L0_2 = nearbythemepark
    if true == L0_2 then
      L0_2 = DoesEntityExist
      L1_2 = gforcehandler
      L1_2 = L1_2.mainhandler
      L0_2 = L0_2(L1_2)
      if L0_2 then
        L0_2 = FreezeEntityPosition
        L1_2 = gforcehandler
        L1_2 = L1_2.mainhandler
        L2_2 = true
        L0_2(L1_2, L2_2)
      else
        L0_2 = GetHashKey
        L1_2 = "sempre_delperropier_gbooster_rameno"
        L0_2 = L0_2(L1_2)
        L1_2 = RequestModel
        L2_2 = L0_2
        L1_2(L2_2)
        while true do
          L1_2 = HasModelLoaded
          L2_2 = L0_2
          L1_2 = L1_2(L2_2)
          if L1_2 then
            break
          end
          L1_2 = RequestModel
          L2_2 = L0_2
          L1_2(L2_2)
          L1_2 = Citizen
          L1_2 = L1_2.Wait
          L2_2 = 5
          L1_2(L2_2)
        end
        L1_2 = gforcehandler
        L2_2 = CreateObjectNoOffset
        L3_2 = L0_2
        L4_2 = gforcehandler
        L4_2 = L4_2.coords
        L4_2 = L4_2.x
        L5_2 = gforcehandler
        L5_2 = L5_2.coords
        L5_2 = L5_2.y
        L6_2 = gforcehandler
        L6_2 = L6_2.coords
        L6_2 = L6_2.z
        L7_2 = false
        L8_2 = true
        L9_2 = true
        L2_2 = L2_2(L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2)
        L1_2.mainhandler = L2_2
        L1_2 = SetEntityRotation
        L2_2 = gforcehandler
        L2_2 = L2_2.mainhandler
        L3_2 = 0.0
        L4_2 = gforcehandler
        L4_2 = L4_2.currentrotation
        L5_2 = 230.0
        L1_2(L2_2, L3_2, L4_2, L5_2)
        L1_2 = NetworkAllowLocalEntityAttachment
        L2_2 = gforcehandler
        L2_2 = L2_2.mainhandler
        L3_2 = true
        L1_2(L2_2, L3_2)
        L1_2 = FreezeEntityPosition
        L2_2 = gforcehandler
        L2_2 = L2_2.mainhandler
        L3_2 = true
        L1_2(L2_2, L3_2)
      end
      L0_2 = DoesEntityExist
      L1_2 = gforcehandler
      L1_2 = L1_2.mainhandler
      L0_2 = L0_2(L1_2)
      if L0_2 then
        L0_2 = DoesEntityExist
        L1_2 = gforcehandler
        L1_2 = L1_2.animhandler
        L0_2 = L0_2(L1_2)
        if L0_2 then
          L0_2 = FreezeEntityPosition
          L1_2 = gforcehandler
          L1_2 = L1_2.animhandler
          L2_2 = true
          L0_2(L1_2, L2_2)
        else
          L0_2 = GetHashKey
          L1_2 = "sempre_delperropier_gbooster_rameno_anim"
          L0_2 = L0_2(L1_2)
          L1_2 = RequestModel
          L2_2 = L0_2
          L1_2(L2_2)
          while true do
            L1_2 = HasModelLoaded
            L2_2 = L0_2
            L1_2 = L1_2(L2_2)
            if L1_2 then
              break
            end
            L1_2 = RequestModel
            L2_2 = L0_2
            L1_2(L2_2)
            L1_2 = Citizen
            L1_2 = L1_2.Wait
            L2_2 = 5
            L1_2(L2_2)
          end
          L1_2 = gforcehandler
          L2_2 = CreateObjectNoOffset
          L3_2 = L0_2
          L4_2 = gforcehandler
          L4_2 = L4_2.coords
          L4_2 = L4_2.x
          L5_2 = gforcehandler
          L5_2 = L5_2.coords
          L5_2 = L5_2.y
          L6_2 = gforcehandler
          L6_2 = L6_2.coords
          L6_2 = L6_2.z
          L7_2 = false
          L8_2 = true
          L9_2 = true
          L2_2 = L2_2(L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2)
          L1_2.animhandler = L2_2
          L1_2 = SetEntityRotation
          L2_2 = gforcehandler
          L2_2 = L2_2.animhandler
          L3_2 = 0.0
          L4_2 = gforcehandler
          L4_2 = L4_2.currentrotation
          L5_2 = 230.0
          L1_2(L2_2, L3_2, L4_2, L5_2)
          L1_2 = NetworkAllowLocalEntityAttachment
          L2_2 = gforcehandler
          L2_2 = L2_2.animhandler
          L3_2 = true
          L1_2(L2_2, L3_2)
          L1_2 = FreezeEntityPosition
          L2_2 = gforcehandler
          L2_2 = L2_2.animhandler
          L3_2 = true
          L1_2(L2_2, L3_2)
          L1_2 = AttachEntityToEntity
          L2_2 = gforcehandler
          L2_2 = L2_2.animhandler
          L3_2 = gforcehandler
          L3_2 = L3_2.mainhandler
          L4_2 = 0
          L5_2 = 0.0
          L6_2 = 0.0
          L7_2 = 0.0
          L8_2 = 0.0
          L9_2 = 0.0
          L10_2 = 0.0
          L11_2 = false
          L12_2 = false
          L13_2 = true
          L14_2 = false
          L15_2 = 2
          L16_2 = true
          L1_2(L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2)
        end
      end
      L0_2 = ipairs
      L1_2 = gforcehandler
      L1_2 = L1_2.seats
      L0_2, L1_2, L2_2, L3_2 = L0_2(L1_2)
      for L4_2, L5_2 in L0_2, L1_2, L2_2, L3_2 do
        L6_2 = DoesEntityExist
        L7_2 = L5_2.seathandlermain
        L6_2 = L6_2(L7_2)
        if L6_2 then
          L6_2 = FreezeEntityPosition
          L7_2 = L5_2.seathandlermain
          L8_2 = true
          L6_2(L7_2, L8_2)
        else
          L6_2 = GetHashKey
          L7_2 = "sempre_delperropier_gbooster_sedacka"
          L6_2 = L6_2(L7_2)
          L7_2 = RequestModel
          L8_2 = L6_2
          L7_2(L8_2)
          while true do
            L7_2 = HasModelLoaded
            L8_2 = L6_2
            L7_2 = L7_2(L8_2)
            if L7_2 then
              break
            end
            L7_2 = RequestModel
            L8_2 = L6_2
            L7_2(L8_2)
            L7_2 = Citizen
            L7_2 = L7_2.Wait
            L8_2 = 5
            L7_2(L8_2)
          end
          L7_2 = CreateObjectNoOffset
          L8_2 = L6_2
          L9_2 = gforcehandler
          L9_2 = L9_2.coords
          L9_2 = L9_2.x
          L10_2 = gforcehandler
          L10_2 = L10_2.coords
          L10_2 = L10_2.y
          L11_2 = gforcehandler
          L11_2 = L11_2.coords
          L11_2 = L11_2.z
          L12_2 = false
          L13_2 = true
          L14_2 = true
          L7_2 = L7_2(L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2)
          L5_2.seathandlermain = L7_2
          L7_2 = SetEntityRotation
          L8_2 = L5_2.seathandlermain
          L9_2 = 0.0
          L10_2 = 0.0
          L11_2 = 230.0
          L7_2(L8_2, L9_2, L10_2, L11_2)
          L7_2 = NetworkAllowLocalEntityAttachment
          L8_2 = L5_2.seathandlermain
          L9_2 = true
          L7_2(L8_2, L9_2)
          L7_2 = FreezeEntityPosition
          L8_2 = L5_2.seathandlermain
          L9_2 = true
          L7_2(L8_2, L9_2)
          if 1 == L4_2 then
            L7_2 = AttachEntityToEntity
            L8_2 = L5_2.seathandlermain
            L9_2 = gforcehandler
            L9_2 = L9_2.mainhandler
            L10_2 = 0
            L11_2 = 0.0
            L12_2 = 0.4
            L13_2 = -26.4
            L14_2 = 0.0
            L15_2 = gforcehandler
            L15_2 = L15_2.currentrotation
            L16_2 = 0.0
            L17_2 = false
            L18_2 = false
            L19_2 = true
            L20_2 = false
            L21_2 = 2
            L22_2 = true
            L7_2(L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2)
          else
            L7_2 = AttachEntityToEntity
            L8_2 = L5_2.seathandlermain
            L9_2 = gforcehandler
            L9_2 = L9_2.mainhandler
            L10_2 = 0
            L11_2 = 0.0
            L12_2 = 0.4
            L13_2 = 26.2
            L14_2 = 0.0
            L15_2 = gforcehandler
            L15_2 = L15_2.currentrotation
            L16_2 = 0.0
            L17_2 = false
            L18_2 = false
            L19_2 = true
            L20_2 = false
            L21_2 = 2
            L22_2 = true
            L7_2(L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2)
          end
        end
        L6_2 = DoesEntityExist
        L7_2 = L5_2.seathandlermain
        L6_2 = L6_2(L7_2)
        if L6_2 then
          L6_2 = DoesEntityExist
          L7_2 = L5_2.animhandler
          L6_2 = L6_2(L7_2)
          if L6_2 then
            L6_2 = FreezeEntityPosition
            L7_2 = L5_2.animhandler
            L8_2 = true
            L6_2(L7_2, L8_2)
          else
            L6_2 = GetHashKey
            L7_2 = "sempre_delperropier_gbooster_sedacka_anim"
            L6_2 = L6_2(L7_2)
            L7_2 = RequestModel
            L8_2 = L6_2
            L7_2(L8_2)
            while true do
              L7_2 = HasModelLoaded
              L8_2 = L6_2
              L7_2 = L7_2(L8_2)
              if L7_2 then
                break
              end
              L7_2 = RequestModel
              L8_2 = L6_2
              L7_2(L8_2)
              L7_2 = Citizen
              L7_2 = L7_2.Wait
              L8_2 = 5
              L7_2(L8_2)
            end
            L7_2 = CreateObjectNoOffset
            L8_2 = L6_2
            L9_2 = gforcehandler
            L9_2 = L9_2.coords
            L9_2 = L9_2.x
            L10_2 = gforcehandler
            L10_2 = L10_2.coords
            L10_2 = L10_2.y
            L11_2 = gforcehandler
            L11_2 = L11_2.coords
            L11_2 = L11_2.z
            L12_2 = false
            L13_2 = true
            L14_2 = true
            L7_2 = L7_2(L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2)
            L5_2.animhandler = L7_2
            L7_2 = SetEntityRotation
            L8_2 = L5_2.animhandler
            L9_2 = 0.0
            L10_2 = 0.0
            L11_2 = 230.0
            L7_2(L8_2, L9_2, L10_2, L11_2)
            L7_2 = NetworkAllowLocalEntityAttachment
            L8_2 = L5_2.animhandler
            L9_2 = true
            L7_2(L8_2, L9_2)
            L7_2 = FreezeEntityPosition
            L8_2 = L5_2.animhandler
            L9_2 = true
            L7_2(L8_2, L9_2)
            L7_2 = AttachEntityToEntity
            L8_2 = L5_2.animhandler
            L9_2 = L5_2.seathandlermain
            L10_2 = 0
            L11_2 = 0.0
            L12_2 = 0.0
            L13_2 = 0.0
            L14_2 = 0.0
            L15_2 = 0.0
            L16_2 = 0.0
            L17_2 = false
            L18_2 = false
            L19_2 = true
            L20_2 = false
            L21_2 = 2
            L22_2 = true
            L7_2(L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2)
          end
        end
        L6_2 = ipairs
        L7_2 = L5_2.seathandlercage
        L6_2, L7_2, L8_2, L9_2 = L6_2(L7_2)
        for L10_2, L11_2 in L6_2, L7_2, L8_2, L9_2 do
          L12_2 = DoesEntityExist
          L13_2 = L11_2.handler
          L12_2 = L12_2(L13_2)
          if L12_2 then
            L12_2 = FreezeEntityPosition
            L13_2 = L11_2.handler
            L14_2 = true
            L12_2(L13_2, L14_2)
          else
            L12_2 = GetHashKey
            L13_2 = "sempre_delperropier_gbooster_zavirani"
            L12_2 = L12_2(L13_2)
            L13_2 = RequestModel
            L14_2 = L12_2
            L13_2(L14_2)
            while true do
              L13_2 = HasModelLoaded
              L14_2 = L12_2
              L13_2 = L13_2(L14_2)
              if L13_2 then
                break
              end
              L13_2 = RequestModel
              L14_2 = L12_2
              L13_2(L14_2)
              L13_2 = Citizen
              L13_2 = L13_2.Wait
              L14_2 = 5
              L13_2(L14_2)
            end
            L13_2 = CreateObjectNoOffset
            L14_2 = L12_2
            L15_2 = gforcehandler
            L15_2 = L15_2.coords
            L15_2 = L15_2.x
            L16_2 = gforcehandler
            L16_2 = L16_2.coords
            L16_2 = L16_2.y
            L17_2 = gforcehandler
            L17_2 = L17_2.coords
            L17_2 = L17_2.z
            L18_2 = false
            L19_2 = true
            L20_2 = true
            L13_2 = L13_2(L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2)
            L11_2.handler = L13_2
            L13_2 = SetEntityRotation
            L14_2 = L11_2.handler
            L15_2 = 0.0
            L16_2 = 0.0
            L17_2 = 230.0
            L13_2(L14_2, L15_2, L16_2, L17_2)
            L13_2 = NetworkAllowLocalEntityAttachment
            L14_2 = L11_2.handler
            L15_2 = true
            L13_2(L14_2, L15_2)
            L13_2 = FreezeEntityPosition
            L14_2 = L11_2.handler
            L15_2 = true
            L13_2(L14_2, L15_2)
            if 1 == L10_2 then
              L13_2 = 0.0
              L14_2 = L5_2.cageclosed
              if false == L14_2 then
                L13_2 = 70.0
              end
              L14_2 = AttachEntityToEntity
              L15_2 = L11_2.handler
              L16_2 = L5_2.seathandlermain
              L17_2 = 0
              L18_2 = 0.24
              L19_2 = -0.68
              L20_2 = 0.9
              L21_2 = 0.0
              L22_2 = L13_2
              L23_2 = -180.0
              L24_2 = false
              L25_2 = false
              L26_2 = false
              L27_2 = false
              L28_2 = 2
              L29_2 = true
              L14_2(L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2, L27_2, L28_2, L29_2)
            else
              L13_2 = 0.0
              L14_2 = L5_2.cageclosed
              if false == L14_2 then
                L13_2 = 70.0
              end
              L14_2 = AttachEntityToEntity
              L15_2 = L11_2.handler
              L16_2 = L5_2.seathandlermain
              L17_2 = 0
              L18_2 = -0.24
              L19_2 = -0.68
              L20_2 = 0.9
              L21_2 = 0.0
              L22_2 = L13_2
              L23_2 = 0.0
              L24_2 = false
              L25_2 = false
              L26_2 = false
              L27_2 = false
              L28_2 = 2
              L29_2 = true
              L14_2(L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2, L26_2, L27_2, L28_2, L29_2)
            end
          end
        end
      end
    end
  end
end
L6_1(L7_1)
L6_1 = -1
L7_1 = Citizen
L7_1 = L7_1.CreateThread
function L8_1()
  local L0_2, L1_2, L2_2, L3_2, L4_2
  while true do
    L0_2 = Citizen
    L0_2 = L0_2.Wait
    L1_2 = 0
    L0_2(L1_2)
    L0_2 = GlobalState
    L0_2 = L0_2["attraction1-phase"]
    if 0 ~= L0_2 then
      L0_2 = nearbythemepark
      if false ~= L0_2 then
        goto lbl_21
      end
    end
    L0_2 = Citizen
    L0_2 = L0_2.Wait
    L1_2 = 500
    L0_2(L1_2)
    L0_2 = gforcehandler
    L0_2.getnew = true
    L0_2 = -1
    L6_1 = L0_2
    goto lbl_43
    ::lbl_21::
    L0_2 = L6_1
    if -1 ~= L0_2 then
      L0_2 = L6_1
      L1_2 = GlobalState
      L1_2 = L1_2["attraction1-synchdata"]
      if not (L0_2 < L1_2) then
        goto lbl_43
      end
    end
    L0_2 = tonumber
    L1_2 = GlobalState
    L1_2 = L1_2["attraction1-synchdata"]
    L0_2 = L0_2(L1_2)
    L6_1 = L0_2
    L0_2 = TriggerEvent
    L1_2 = "rtx_themepark:GForce:SynchronizeMovement"
    L2_2 = GlobalState
    L2_2 = L2_2["attraction1-ridedata1"]
    L3_2 = GlobalState
    L3_2 = L3_2["attraction1-speeddata1"]
    L4_2 = GlobalState
    L4_2 = L4_2["attraction1-phase"]
    L0_2(L1_2, L2_2, L3_2, L4_2)
    ::lbl_43::
  end
end
L7_1(L8_1)
L7_1 = Config
L7_1 = L7_1.AttractionsSettings
L7_1 = L7_1.gforce
L7_1 = L7_1.disable
if false == L7_1 then
  L7_1 = Citizen
  L7_1 = L7_1.CreateThread
  function L8_1()
    local L0_2, L1_2, L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2
    while true do
      L0_2 = Citizen
      L0_2 = L0_2.Wait
      L1_2 = 0
      L0_2(L1_2)
      L0_2 = true
      L1_2 = false
      L2_2 = -1
      L3_2 = {}
      L3_2.seatcategoryid = nil
      L3_2.seatid = nil
      L4_2 = nearbythemepark
      if true == L4_2 then
        L4_2 = tickets
        L4_2 = L4_2.gforce
        if true == L4_2 then
          L4_2 = gforcehandler
          L4_2 = L4_2.started
          if false == L4_2 then
            L4_2 = gforcehandler
            L4_2 = L4_2.changingsides
            if false == L4_2 then
              L4_2 = ipairs
              L5_2 = gforcehandler
              L5_2 = L5_2.seats
              L4_2, L5_2, L6_2, L7_2 = L4_2(L5_2)
              for L8_2, L9_2 in L4_2, L5_2, L6_2, L7_2 do
                L10_2 = gforcehandler
                L10_2 = L10_2.seatdown
                if L10_2 == L8_2 then
                  L10_2 = ipairs
                  L11_2 = L9_2.seats
                  L10_2, L11_2, L12_2, L13_2 = L10_2(L11_2)
                  for L14_2, L15_2 in L10_2, L11_2, L12_2, L13_2 do
                    L16_2 = L15_2.taken
                    if false == L16_2 then
                      L16_2 = GetOffsetFromEntityInWorldCoords
                      L17_2 = L9_2.seathandlermain
                      L18_2 = L15_2.offsets
                      L18_2 = L18_2.coords
                      L18_2 = L18_2.x
                      L19_2 = L15_2.offsets
                      L19_2 = L19_2.coords
                      L19_2 = L19_2.y
                      L20_2 = L15_2.offsets
                      L20_2 = L20_2.coords
                      L20_2 = L20_2.z
                      L16_2 = L16_2(L17_2, L18_2, L19_2, L20_2)
                      L17_2 = playercurrentcoords
                      L17_2 = L17_2 - L16_2
                      L17_2 = #L17_2
                      if L17_2 < 20.0 then
                        L18_2 = Config
                        L18_2 = L18_2.AttractionsSettings
                        L18_2 = L18_2.gforce
                        L18_2 = L18_2.usedistance
                        if L17_2 < L18_2 and (-1 == L2_2 or L2_2 > L17_2) then
                          L2_2 = L17_2
                          L1_2 = true
                          L3_2.seatcategoryid = L8_2
                          L3_2.seatid = L14_2
                        end
                      end
                    end
                  end
                end
              end
            end
          end
        end
        if L1_2 then
          L4_2 = {}
          L5_2 = L3_2.seatcategoryid
          L4_2.seatcategoryid = L5_2
          L5_2 = L3_2.seatid
          L4_2.seatid = L5_2
          L0_1 = L4_2
          L4_2 = false
          L5_2 = usingattraction
          if false == L5_2 then
            L0_2 = false
            L5_2 = Config
            L5_2 = L5_2.Target
            if false == L5_2 then
              L5_2 = gforcehandler
              L5_2 = L5_2.seats
              L6_2 = L0_1.seatcategoryid
              L5_2 = L5_2[L6_2]
              L6_2 = L5_2.seats
              L7_2 = L0_1.seatid
              L6_2 = L6_2[L7_2]
              L7_2 = Config
              L7_2 = L7_2.ThemeParkInteractionSystem
              if 1 == L7_2 then
                L7_2 = SendNUIMessage
                L8_2 = {}
                L8_2.message = "infonotifyshow"
                L9_2 = Language
                L10_2 = Config
                L10_2 = L10_2.Language
                L9_2 = L9_2[L10_2]
                L9_2 = L9_2.pressforuseseatinteract
                L8_2.infonotifytext = L9_2
                L7_2(L8_2)
                L4_2 = true
              else
                L7_2 = Config
                L7_2 = L7_2.ThemeParkInteractionSystem
                if 2 == L7_2 then
                  L7_2 = GetOffsetFromEntityInWorldCoords
                  L8_2 = L5_2.seathandlermain
                  L9_2 = L6_2.offsets
                  L9_2 = L9_2.coords
                  L9_2 = L9_2.x
                  L10_2 = L6_2.offsets
                  L10_2 = L10_2.coords
                  L10_2 = L10_2.y
                  L11_2 = L6_2.offsets
                  L11_2 = L11_2.coords
                  L11_2 = L11_2.z
                  L7_2 = L7_2(L8_2, L9_2, L10_2, L11_2)
                  L8_2 = DrawText3D
                  L9_2 = L7_2.x
                  L10_2 = L7_2.y
                  L11_2 = L7_2.z
                  L12_2 = Language
                  L13_2 = Config
                  L13_2 = L13_2.Language
                  L12_2 = L12_2[L13_2]
                  L12_2 = L12_2.pressforuseseat
                  L8_2(L9_2, L10_2, L11_2, L12_2)
                else
                  L7_2 = Config
                  L7_2 = L7_2.ThemeParkInteractionSystem
                  if 3 == L7_2 then
                    L7_2 = ShowGtaClassicInteraction
                    L8_2 = Language
                    L9_2 = Config
                    L9_2 = L9_2.Language
                    L8_2 = L8_2[L9_2]
                    L8_2 = L8_2.pressforuseseatinteractclassic
                    L7_2(L8_2)
                  end
                end
              end
            end
          end
        else
          L4_2 = Config
          L4_2 = L4_2.ThemeParkInteractionSystem
          if 1 == L4_2 then
            L4_2 = L0_1.seatcategoryid
            if nil ~= L4_2 then
              L4_2 = SendNUIMessage
              L5_2 = {}
              L5_2.message = "hide"
              L4_2(L5_2)
            end
          end
          L4_2 = {}
          L4_2.seatcategoryid = nil
          L4_2.seatid = nil
          L0_1 = L4_2
        end
      end
      if L0_2 then
        L4_2 = Citizen
        L4_2 = L4_2.Wait
        L5_2 = 1000
        L4_2(L5_2)
      end
    end
  end
  L7_1(L8_1)
end
L7_1 = Config
L7_1 = L7_1.ThemeParkAttractionFallChance
if L7_1 then
  L7_1 = Config
  L7_1 = L7_1.ThemeParkFallSettings
  L7_1 = L7_1.attractions
  L7_1 = L7_1.gforce
  if L7_1 then
    L7_1 = Citizen
    L7_1 = L7_1.CreateThread
    function L8_1()
      local L0_2, L1_2, L2_2, L3_2, L4_2
      while true do
        L0_2 = Citizen
        L0_2 = L0_2.Wait
        L1_2 = 0
        L0_2(L1_2)
        L0_2 = nearbythemepark
        if true == L0_2 then
          L0_2 = L1_1
          if nil ~= L0_2 then
            L0_2 = L2_1
            if nil ~= L0_2 then
              L0_2 = L4_1
              if true == L0_2 then
                L0_2 = gforcehandler
                L0_2 = L0_2.started
                if true == L0_2 then
                  L0_2 = L5_1
                  if L0_2 > 0 then
                    L0_2 = L5_1
                    L0_2 = L0_2 - 1
                    L5_1 = L0_2
                    L0_2 = Citizen
                    L0_2 = L0_2.Wait
                    L1_2 = 1000
                    L0_2(L1_2)
                  else
                    L0_2 = Citizen
                    L0_2 = L0_2.Wait
                    L1_2 = 1000
                    L0_2(L1_2)
                    L0_2 = math
                    L0_2 = L0_2.random
                    L1_2 = 1000
                    L0_2 = L0_2(L1_2)
                    L1_2 = Config
                    L1_2 = L1_2.ThemeParkFallSettings
                    L1_2 = L1_2.fallchance
                    if L0_2 <= L1_2 then
                      L1_2 = false
                      L4_1 = L1_2
                      L1_2 = usingattraction
                      if true == L1_2 then
                        L1_2 = L1_1
                        if nil ~= L1_2 then
                          L1_2 = L2_1
                          if nil ~= L1_2 then
                            L1_2 = TriggerServerEvent
                            L2_2 = "rtx_themepark:GForce:ThrowAttraction"
                            L3_2 = L1_1
                            L4_2 = L2_1
                            L1_2(L2_2, L3_2, L4_2)
                          end
                        end
                      end
                    else
                      L1_2 = Config
                      L1_2 = L1_2.ThemeParkFallSettings
                      L1_2 = L1_2.fallchancecheck
                      L5_1 = L1_2
                    end
                  end
              end
            end
          end
          else
            L0_2 = Citizen
            L0_2 = L0_2.Wait
            L1_2 = 1500
            L0_2(L1_2)
          end
        else
          L0_2 = Citizen
          L0_2 = L0_2.Wait
          L1_2 = 1500
          L0_2(L1_2)
        end
      end
    end
    L7_1(L8_1)
  end
end
L7_1 = RegisterNetEvent
L8_1 = "rtx_themepark:GForce:SynchronizeSeat"
L7_1(L8_1)
L7_1 = AddEventHandler
L8_1 = "rtx_themepark:GForce:SynchronizeSeat"
function L9_1(A0_2, A1_2, A2_2, A3_2, A4_2)
  local L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2
  L5_2 = gforcehandler
  L5_2 = L5_2.seats
  L5_2 = L5_2[A0_2]
  L6_2 = L5_2.seats
  L6_2 = L6_2[A1_2]
  L6_2.taken = A2_2
  if false == A2_2 then
    if nil ~= A3_2 then
      L7_2 = GetPlayerFromServerId
      L8_2 = A3_2
      L7_2 = L7_2(L8_2)
      if -1 ~= L7_2 then
        L8_2 = GetPlayerPed
        L9_2 = L7_2
        L8_2 = L8_2(L9_2)
        L9_2 = DoesEntityExist
        L10_2 = L8_2
        L9_2 = L9_2(L10_2)
        if L9_2 then
          L9_2 = DetachEntity
          L10_2 = L8_2
          L9_2(L10_2)
          L9_2 = FreezeEntityPosition
          L10_2 = L8_2
          L11_2 = false
          L9_2(L10_2, L11_2)
          L9_2 = ClearPedTasks
          L10_2 = L8_2
          L9_2(L10_2)
        end
      end
    end
  else
    L7_2 = GetPlayerFromServerId
    L8_2 = A3_2
    L7_2 = L7_2(L8_2)
    if -1 ~= L7_2 then
      L8_2 = GetPlayerPed
      L9_2 = L7_2
      L8_2 = L8_2(L9_2)
      L9_2 = DoesEntityExist
      L10_2 = L8_2
      L9_2 = L9_2(L10_2)
      if L9_2 then
        if 2 == A4_2 then
          L9_2 = FreezeEntityPosition
          L10_2 = L8_2
          L11_2 = true
          L9_2(L10_2, L11_2)
          L9_2 = NetworkAllowLocalEntityAttachment
          L10_2 = L8_2
          L11_2 = true
          L9_2(L10_2, L11_2)
          L9_2 = AttachEntityToEntity
          L10_2 = L8_2
          L11_2 = L5_2.seathandlermain
          L12_2 = 0
          L13_2 = L6_2.offsets
          L13_2 = L13_2.coords
          L13_2 = L13_2.x
          L14_2 = L6_2.offsets
          L14_2 = L14_2.coords
          L14_2 = L14_2.y
          L15_2 = L6_2.offsets
          L15_2 = L15_2.coords
          L15_2 = L15_2.z
          L15_2 = L15_2 + 0.1
          L16_2 = 0.0
          L17_2 = 0.0
          L18_2 = L6_2.offsets
          L18_2 = L18_2.heading
          L19_2 = false
          L20_2 = false
          L21_2 = true
          L22_2 = false
          L23_2 = 2
          L24_2 = true
          L9_2(L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2)
          L9_2 = "rtxthemepark"
          L10_2 = "rtxhandsup_clip"
          while true do
            L11_2 = HasAnimDictLoaded
            L12_2 = L9_2
            L11_2 = L11_2(L12_2)
            if L11_2 then
              break
            end
            L11_2 = RequestAnimDict
            L12_2 = L9_2
            L11_2(L12_2)
            L11_2 = Citizen
            L11_2 = L11_2.Wait
            L12_2 = 5
            L11_2(L12_2)
          end
          L11_2 = TaskPlayAnim
          L12_2 = L8_2
          L13_2 = L9_2
          L14_2 = L10_2
          L15_2 = 8.0
          L16_2 = 8.0
          L17_2 = -1
          L18_2 = 1
          L19_2 = 0
          L20_2 = 0
          L21_2 = 0
          L22_2 = 0
          L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2)
        else
          L9_2 = FreezeEntityPosition
          L10_2 = L8_2
          L11_2 = true
          L9_2(L10_2, L11_2)
          L9_2 = NetworkAllowLocalEntityAttachment
          L10_2 = L8_2
          L11_2 = true
          L9_2(L10_2, L11_2)
          L9_2 = AttachEntityToEntity
          L10_2 = L8_2
          L11_2 = L5_2.seathandlermain
          L12_2 = 0
          L13_2 = L6_2.offsets
          L13_2 = L13_2.coords
          L13_2 = L13_2.x
          L14_2 = L6_2.offsets
          L14_2 = L14_2.coords
          L14_2 = L14_2.y
          L15_2 = L6_2.offsets
          L15_2 = L15_2.coords
          L15_2 = L15_2.z
          L16_2 = 0.0
          L17_2 = 0.0
          L18_2 = L6_2.offsets
          L18_2 = L18_2.heading
          L19_2 = false
          L20_2 = false
          L21_2 = true
          L22_2 = false
          L23_2 = 2
          L24_2 = true
          L9_2(L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2)
          L9_2 = "amb@prop_human_seat_chair_mp@female@proper@idle_a"
          L10_2 = "idle_b"
          while true do
            L11_2 = HasAnimDictLoaded
            L12_2 = L9_2
            L11_2 = L11_2(L12_2)
            if L11_2 then
              break
            end
            L11_2 = RequestAnimDict
            L12_2 = L9_2
            L11_2(L12_2)
            L11_2 = Citizen
            L11_2 = L11_2.Wait
            L12_2 = 5
            L11_2(L12_2)
          end
          L11_2 = TaskPlayAnim
          L12_2 = L8_2
          L13_2 = L9_2
          L14_2 = L10_2
          L15_2 = 8.0
          L16_2 = 8.0
          L17_2 = -1
          L18_2 = 1
          L19_2 = 0
          L20_2 = 0
          L21_2 = 0
          L22_2 = 0
          L11_2(L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2)
        end
      end
    end
  end
end
L7_1(L8_1, L9_1)
L7_1 = RegisterNetEvent
L8_1 = "rtx_themepark:GForce:SynchronizeCageClient"
L7_1(L8_1)
L7_1 = AddEventHandler
L8_1 = "rtx_themepark:GForce:SynchronizeCageClient"
function L9_1(A0_2, A1_2)
  local L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2
  L2_2 = gforcehandler
  L2_2 = L2_2.seats
  L2_2 = L2_2[A0_2]
  L2_2.cageclosed = A1_2
  if true == A1_2 then
    L3_2 = 0.0
    while L3_2 < 70.0 do
      L4_2 = Citizen
      L4_2 = L4_2.Wait
      L5_2 = 10
      L4_2(L5_2)
      L3_2 = L3_2 + 0.25
      L4_2 = ipairs
      L5_2 = L2_2.seathandlercage
      L4_2, L5_2, L6_2, L7_2 = L4_2(L5_2)
      for L8_2, L9_2 in L4_2, L5_2, L6_2, L7_2 do
        L10_2 = DoesEntityExist
        L11_2 = L9_2.handler
        L10_2 = L10_2(L11_2)
        if L10_2 then
          if 1 == L8_2 then
            L10_2 = AttachEntityToEntity
            L11_2 = L9_2.handler
            L12_2 = L2_2.seathandlermain
            L13_2 = 0
            L14_2 = 0.24
            L15_2 = -0.68
            L16_2 = 0.9
            L17_2 = 0.0
            L18_2 = L3_2
            L19_2 = -180.0
            L20_2 = false
            L21_2 = false
            L22_2 = false
            L23_2 = false
            L24_2 = 2
            L25_2 = true
            L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
          else
            L10_2 = AttachEntityToEntity
            L11_2 = L9_2.handler
            L12_2 = L2_2.seathandlermain
            L13_2 = 0
            L14_2 = -0.24
            L15_2 = -0.68
            L16_2 = 0.9
            L17_2 = 0.0
            L18_2 = L3_2
            L19_2 = 0.0
            L20_2 = false
            L21_2 = false
            L22_2 = false
            L23_2 = false
            L24_2 = 2
            L25_2 = true
            L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
          end
        end
      end
    end
    L4_2 = ipairs
    L5_2 = L2_2.seathandlercage
    L4_2, L5_2, L6_2, L7_2 = L4_2(L5_2)
    for L8_2, L9_2 in L4_2, L5_2, L6_2, L7_2 do
      L10_2 = DoesEntityExist
      L11_2 = L9_2.handler
      L10_2 = L10_2(L11_2)
      if L10_2 then
        if 1 == L8_2 then
          L10_2 = AttachEntityToEntity
          L11_2 = L9_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = 0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = 70.0
          L19_2 = -180.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        else
          L10_2 = AttachEntityToEntity
          L11_2 = L9_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = -0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = 70.0
          L19_2 = 0.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        end
      end
    end
  else
    L3_2 = 70.0
    while L3_2 > 0.0 do
      L4_2 = Citizen
      L4_2 = L4_2.Wait
      L5_2 = 10
      L4_2(L5_2)
      L3_2 = L3_2 - 0.25
      L4_2 = ipairs
      L5_2 = L2_2.seathandlercage
      L4_2, L5_2, L6_2, L7_2 = L4_2(L5_2)
      for L8_2, L9_2 in L4_2, L5_2, L6_2, L7_2 do
        L10_2 = DoesEntityExist
        L11_2 = L9_2.handler
        L10_2 = L10_2(L11_2)
        if L10_2 then
          if 1 == L8_2 then
            L10_2 = AttachEntityToEntity
            L11_2 = L9_2.handler
            L12_2 = L2_2.seathandlermain
            L13_2 = 0
            L14_2 = 0.24
            L15_2 = -0.68
            L16_2 = 0.9
            L17_2 = 0.0
            L18_2 = L3_2
            L19_2 = -180.0
            L20_2 = false
            L21_2 = false
            L22_2 = false
            L23_2 = false
            L24_2 = 2
            L25_2 = true
            L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
          else
            L10_2 = AttachEntityToEntity
            L11_2 = L9_2.handler
            L12_2 = L2_2.seathandlermain
            L13_2 = 0
            L14_2 = -0.24
            L15_2 = -0.68
            L16_2 = 0.9
            L17_2 = 0.0
            L18_2 = L3_2
            L19_2 = 0.0
            L20_2 = false
            L21_2 = false
            L22_2 = false
            L23_2 = false
            L24_2 = 2
            L25_2 = true
            L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
          end
        end
      end
    end
    L4_2 = ipairs
    L5_2 = L2_2.seathandlercage
    L4_2, L5_2, L6_2, L7_2 = L4_2(L5_2)
    for L8_2, L9_2 in L4_2, L5_2, L6_2, L7_2 do
      L10_2 = DoesEntityExist
      L11_2 = L9_2.handler
      L10_2 = L10_2(L11_2)
      if L10_2 then
        if 1 == L8_2 then
          L10_2 = AttachEntityToEntity
          L11_2 = L9_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = 0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = 0.0
          L19_2 = -180.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        else
          L10_2 = AttachEntityToEntity
          L11_2 = L9_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = -0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = 0.0
          L19_2 = 0.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        end
      end
    end
  end
end
L7_1(L8_1, L9_1)
L7_1 = RegisterNetEvent
L8_1 = "rtx_themepark:GForce:SynchronizeCageClientResync"
L7_1(L8_1)
L7_1 = AddEventHandler
L8_1 = "rtx_themepark:GForce:SynchronizeCageClientResync"
function L9_1(A0_2, A1_2)
  local L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2, L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2
  L2_2 = gforcehandler
  L2_2 = L2_2.seats
  L2_2 = L2_2[A0_2]
  L2_2.cageclosed = A1_2
  if true == A1_2 then
    L3_2 = ipairs
    L4_2 = L2_2.seathandlercage
    L3_2, L4_2, L5_2, L6_2 = L3_2(L4_2)
    for L7_2, L8_2 in L3_2, L4_2, L5_2, L6_2 do
      L9_2 = DoesEntityExist
      L10_2 = L8_2.handler
      L9_2 = L9_2(L10_2)
      if L9_2 then
        if 1 == L7_2 then
          L9_2 = 0.0
          if false == A1_2 then
            L9_2 = 70.0
          end
          L10_2 = AttachEntityToEntity
          L11_2 = L8_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = 0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = L9_2
          L19_2 = -180.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        else
          L9_2 = 0.0
          if false == A1_2 then
            L9_2 = 70.0
          end
          L10_2 = AttachEntityToEntity
          L11_2 = L8_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = -0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = L9_2
          L19_2 = 0.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        end
      end
    end
  else
    L3_2 = ipairs
    L4_2 = L2_2.seathandlercage
    L3_2, L4_2, L5_2, L6_2 = L3_2(L4_2)
    for L7_2, L8_2 in L3_2, L4_2, L5_2, L6_2 do
      L9_2 = DoesEntityExist
      L10_2 = L8_2.handler
      L9_2 = L9_2(L10_2)
      if L9_2 then
        if 1 == L7_2 then
          L9_2 = 0.0
          if false == A1_2 then
            L9_2 = 70.0
          end
          L10_2 = AttachEntityToEntity
          L11_2 = L8_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = 0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = L9_2
          L19_2 = -180.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        else
          L9_2 = 0.0
          if false == A1_2 then
            L9_2 = 70.0
          end
          L10_2 = AttachEntityToEntity
          L11_2 = L8_2.handler
          L12_2 = L2_2.seathandlermain
          L13_2 = 0
          L14_2 = -0.24
          L15_2 = -0.68
          L16_2 = 0.9
          L17_2 = 0.0
          L18_2 = L9_2
          L19_2 = 0.0
          L20_2 = false
          L21_2 = false
          L22_2 = false
          L23_2 = false
          L24_2 = 2
          L25_2 = true
          L10_2(L11_2, L12_2, L13_2, L14_2, L15_2, L16_2, L17_2, L18_2, L19_2, L20_2, L21_2, L22_2, L23_2, L24_2, L25_2)
        end
      end
    end
  end
end
L7_1(L8_1, L9_1)
L7_1 = Config
L7_1 = L7_1.Target
if false == L7_1 then
  L7_1 = RegisterCommand
  L8_1 = "usegforceseat"
  function L9_1()
    local L0_2, L1_2, L2_2, L3_2
    L0_2 = usingattraction
    if false == L0_2 then
      L0_2 = L0_1.seatcategoryid
      if nil ~= L0_2 then
        L0_2 = L0_1.seatid
        if nil ~= L0_2 then
          L0_2 = iteminhand
          if false == L0_2 then
            L0_2 = TriggerServerEvent
            L1_2 = "rtx_themepark:GForce:SeatUse"
            L2_2 = L0_1.seatcategoryid
            L3_2 = L0_1.seatid
            L0_2(L1_2, L2_2, L3_2)
          else
            L0_2 = Notify
            L1_2 = Language
            L2_2 = Config
            L2_2 = L2_2.Language
            L1_2 = L1_2[L2_2]
            L1_2 = L1_2.iteminhand
            L0_2(L1_2)
          end
        end
      end
    end
  end
  L7_1(L8_1, L9_1)
  L7_1 = RegisterKeyMapping
  L8_1 = "usegforceseat"
  L9_1 = Language
  L10_1 = Config
  L10_1 = L10_1.Language
  L9_1 = L9_1[L10_1]
  L9_1 = L9_1.bindgforceseatuse
  L10_1 = "keyboard"
  L11_1 = Config
  L11_1 = L11_1.ThemeParkSeatKey
  L7_1(L8_1, L9_1, L10_1, L11_1)
end
L7_1 = RegisterCommand
L8_1 = "changegforceanim"
function L9_1()
  local L0_2, L1_2, L2_2, L3_2
  L0_2 = usingattraction
  if true == L0_2 then
    L0_2 = L1_1
    if nil ~= L0_2 then
      L0_2 = L2_1
      if nil ~= L0_2 then
        L0_2 = L3_1
        if false == L0_2 then
          L0_2 = true
          L3_1 = L0_2
          L0_2 = TriggerServerEvent
          L1_2 = "rtx_themepark:GForce:SeatAnimChange"
          L2_2 = L1_1
          L3_2 = L2_1
          L0_2(L1_2, L2_2, L3_2)
          L0_2 = Citizen
          L0_2 = L0_2.Wait
          L1_2 = Config
          L1_2 = L1_2.AttractionsSettings
          L1_2 = L1_2.gforce
          L1_2 = L1_2.animcooldown
          L0_2(L1_2)
          L0_2 = false
          L3_1 = L0_2
        end
      end
    end
  end
end
L7_1(L8_1, L9_1)
L7_1 = RegisterKeyMapping
L8_1 = "changegforceanim"
L9_1 = Language
L10_1 = Config
L10_1 = L10_1.Language
L9_1 = L9_1[L10_1]
L9_1 = L9_1.bindattractionanimchange
L10_1 = "keyboard"
L11_1 = Config
L11_1 = L11_1.ThemeParkAnimChangeKey
L7_1(L8_1, L9_1, L10_1, L11_1)
L7_1 = RegisterCommand
L8_1 = "exitgforce"
function L9_1()
  local L0_2, L1_2, L2_2, L3_2
  L0_2 = usingattraction
  if true == L0_2 then
    L0_2 = L1_1
    if nil ~= L0_2 then
      L0_2 = L2_1
      if nil ~= L0_2 then
        L0_2 = Config
        L0_2 = L0_2.ThemeParkDisableExit
        if false ~= L0_2 then
          L0_2 = gforcehandler
          L0_2 = L0_2.started
          if false ~= L0_2 then
            goto lbl_24
          end
        end
        L0_2 = TriggerServerEvent
        L1_2 = "rtx_themepark:GForce:ExitAttraction"
        L2_2 = L1_1
        L3_2 = L2_1
        L0_2(L1_2, L2_2, L3_2)
        goto lbl_31
        ::lbl_24::
        L0_2 = Notify
        L1_2 = Language
        L2_2 = Config
        L2_2 = L2_2.Language
        L1_2 = L1_2[L2_2]
        L1_2 = L1_2.inprogress
        L0_2(L1_2)
      end
    end
  end
  ::lbl_31::
end
L7_1(L8_1, L9_1)
L7_1 = RegisterKeyMapping
L8_1 = "exitgforce"
L9_1 = Language
L10_1 = Config
L10_1 = L10_1.Language
L9_1 = L9_1[L10_1]
L9_1 = L9_1.bindattractionexitkey
L10_1 = "keyboard"
L11_1 = Config
L11_1 = L11_1.ThemeParkExitKey
L7_1(L8_1, L9_1, L10_1, L11_1)
