...
--     end
-- end)
RegisterNetEvent('qb-admin:server:inventory', function(targetId)
    local src = source
    TriggerClientEvent('qb-admin:client:inventory', src, targetId)
end)
...
RegisterNetEvent('qb-admin:server:cloth', function(player)
    ...
    if QBCore.Functions.HasPermission(src, permissions['clothing']) or IsPlayerAceAllowed(src, 'command') then
        TriggerClientEvent('sh-creation:client:openMenu', player)
