...
function client.openInventory(inv, data)
    ...
    local left, right, accessError, other
    ...
    if inv == 'player' and data ~= cache.serverId then
        ...
        if not targetCoords or #(targetCoords - GetEntityCoords(playerPed)) > 2.5 or (not client.hasGroup(shared.police) and not Player(serverId).state.canSteal) then
            return lib.notify({
    ...
    elseif invOpen ~= nil then
        ...
        left, right, accessError, other = lib.callback.await('ox_inventory:openInventory', false, inv, data)
    end
    ...
    for i = 1, #setViewType, 1 do
        if inv == setViewType[i] then
            SendNUIMessage({
                action = 'setView',
                data = 'inventory'
            })
...
local function useItem(data, cb, noAnim)
    if IsArmour(data) and not IsItemType(data, 'armor') then
        return lib.notify({ id = 'cannot_perform', type = 'error', description = locale('cannot_perform') })
    end
...
local function setStateBagHandler(stateId)
    AddStateBagChangeHandler('invOpen', stateId, function(_, _, value)
        invOpen = value
        if invOpen and CheckArmour then
            CheckArmour(true)
        end
    end)
