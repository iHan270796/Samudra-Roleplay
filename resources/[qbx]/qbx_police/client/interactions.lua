...
local function getClosestPlayer(distance)
    local coords = GetEntityCoords(cache.ped)
    local player, playerPed = lib.getClosestPlayer(coords, distance or 3.5)
    if not player then
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if not (IsEntityPlayingAnim(playerPed, 'random@mugging3', 'handsup_standing_base', 3)
