...
RegisterNetEvent('qb-k9:client:menu', function()
    lib.registerContext({
        id = 'k9_menu',
        title = Lang:t("menu.purchase_header"),
        options = {
            {
                title = Lang:t("menu.k9_takeout"),
                description = Lang:t("menu.takeout_txt"),
                icon = 'dog', -- opsional, bisa diganti atau dihapus
                onSelect = function()
                    TriggerEvent('qb-k9:client:PurchaseDog')
                end
            },
            {
                title = Lang:t("menu.k9_return"),
                description = Lang:t("menu.return_txt"),
                icon = 'arrow-left', -- opsional
                onSelect = function()
                    TriggerEvent('qb-k9:client:ReturnDoggo')
                end
            },
        }
    })
...
RegisterNetEvent('qb-k9:client:PurchaseDog', function()
    ...
    for k, v in pairs(Config.DogModelProps) do
        K9Dawgs[#K9Dawgs + 1] = {
            ...
            icon = 'dog', -- opsional
            onSelect = function()
                TriggerEvent('qb-k9:client:SpawnHandler', {
                    model = v["Dog"],
                    colour = v["Colour"],
                    vest = v["Vest"]
                })
    ...
    lib.registerContext({
        id = 'purchase_k9_dog_menu',
        title = Lang:t("menu.k9_choose_dog"), -- atau ganti sesuai kebutuhan
        options = K9Dawgs
    })

    lib.showContext('purchase_k9_dog_menu')
end)
...
RegisterNetEvent('K9:CLIENT:SPAWN_K9', function(DawgHash, colour, vest)
  ...
  local DOG = NetworkGetEntityFromNetworkId(k9_id)
  --set K9 Properties
  if GetNumberOfPedDrawableVariations(DOG, 0) > 0 and GetNumberOfPedTextureVariations(DOG, 0, 0) > colour then
    SetPedComponentVariation(DOG, 0, 0, colour, 0)
  end

  if GetNumberOfPedDrawableVariations(DOG, vest) > 0 and GetNumberOfPedTextureVariations(DOG, vest, 0) > 1 then
...
RegisterNetEvent("elixir-k9:Follow", function()
ExecuteCommand('cannineattackfollow')
end)

RegisterNetEvent("elixir-k9:Sit", function()
  PlayAnimation(sit.dict, sit.anim)
end)

RegisterNetEvent("elixir-k9:EnterVeh", function()
  K9ToggleVehicle(false)
end)

RegisterNetEvent("elixir-k9:laydown", function()
  PlayAnimation(laydown.dict, laydown.anim)
end)

RegisterNetEvent("elixir-k9:searchcar", function()
  K9SearchVehicle()
end)

RegisterNetEvent("elixir-k9:searchdude", function()
  K9SearchPerson()
end)

RegisterNetEvent("elixir-k9:searcharea", function()
  K9SearchArea()
end)
