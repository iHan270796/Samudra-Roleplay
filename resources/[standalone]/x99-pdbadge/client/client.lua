...
-- exports['qb-target']:AddBoxZone("badge_target", vector3(72.49, -425.09, 41.5), 1.2, 1.2, {
--     name = "badge_target",
--     heading = 0,
--     debugPoly = false,
--     minZ = 39.0,
...
exports.ox_target:addBoxZone({
    coords = vec3(72.49, -425.09, 41.5),
    size = vec3(1.2, 1.2, 3.2), -- width, length, height (maxZ - minZ)
    rotation = 0,
    debug = false,
    options = {
        {
            ...
            icon = "fas fa-camera",
            label = "Create Photo",
            groups = { "police" }, -- 'job' di qb-target jadi 'groups' di ox_target
        },
        {
            type = "client",
            event = "x99_badge:create",
...
RegisterNetEvent("x99_badge:create", function()
    PlayerData = QBCore.Functions.GetPlayerData()
...
RegisterNetEvent("x99-pdbadge:open", function(sourceId, sender_coords, data)
    if PlayerData.job and PlayerData.job.name then 
        PlayerLoaded = true
    end

    if PlayerLoaded then
        if platespawned then DeleteEntity(platespawned) end
        if plate_net then DeleteEntity(plate_net) end

        local mypos = GetEntityCoords(PlayerPedId())
        if (GetDistanceBetweenCoords(mypos, sender_coords, true) < config.badgedistance) then
            SendNUIMessage({
                action = "badgeOpen",
                name = data.name,
            ...
            while badge do
                ...
                if IsControlJustPressed(1, config.closeKey) then
                    ...
                    badge = false
                    SendNUIMessage({ action = "badgeClose" })
...
function getBadgeType(type)
    local model = "x99_policebadge_lspd"
    if type == "police" then
...
RegisterNetEvent("x99_badge:changeurl", function()
    local input = lib.inputDialog("PD Badge", {
        {type = "input", label = "URL Foto", required = false, icon = "image"}
    })

    if not input or not input[1] then return end
    local PlayerData = QBCore.Functions.GetPlayerData()
    PlayerData.metadata.phonedata = PlayerData.metadata.phonedata or {}
    PlayerData.metadata.phonedata.profilepicture = input[1]
