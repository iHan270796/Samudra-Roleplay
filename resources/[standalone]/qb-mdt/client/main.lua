...
CreateThread(function()
    if GetResourceState('qb-dispatch') == 'started' then
        TriggerServerEvent("qb-mdt:dispatchStatus", true)
    end
end)
...
RegisterNetEvent("QBCore:Client:SetDuty", function(job, state)
	...
	TriggerServerEvent("qb-mdt:server:ClockSystem")
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        exports["qb-voice"]:setRadioChannel(tonumber(radio))
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('qb-dispatch:client:removeCallBlip', data.callid or data.id)
    cb(true)
end)
...
RegisterNetEvent('qb-mdt:client:selfregister', function()
...
RegisterNetEvent("mdt:receiveOfficerData", function(officerData)
    SendNUIMessage({
...
if Config.UseWolfknightRadar == true then
    RegisterNetEvent("qb-mdt:client:trafficStop", function()
