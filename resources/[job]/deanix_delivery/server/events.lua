RegisterNetEvent("deanix_delivery:Server:HandleCallback", function(key, ...)
    local src = source
    if Koci.Callbacks[key] then
        Koci.Callbacks[key](src, function(...)
            TriggerClientEvent("deanix_delivery:Client:HandleCallback", src, key, ...)
        end, ...)
    end
end)

RegisterNetEvent("deanix_delivery:Server:UpdatePlayerProfile", function(newProfile)
    local src = source
    Koci.Server.Functions.UpdateProfile(src, newProfile)
end)

RegisterNetEvent("deanix_delivery:Server:PlayerDeclinedInvitation", function(inviting)
...
RegisterNetEvent("deanix_delivery:Server:LeaveAllTeams", function()
    ...
    if fxPlayer then
        ...
        for _, mate in pairs(teamMates) do
            ...
            if fxTarget then
                TriggerClientEvent("deanix_delivery:Client:PlayerLeaveYourTeam",
                    mateSource,
                    xPlayerSource,
                    xPlayerName
                )
...
RegisterNetEvent("deanix_delivery:Server:UpdateCurrentTaskCustomers", function(newDestinations)
    local xPlayerSource = source
    local fxPlayer = Koci.Server.Functions.GetPlayer(xPlayerSource)
    if fxPlayer then
        local teamMates = Koci.Utils.deepCopy(fxPlayer.PlayerData.TeamMate or {})
        for _, mate in pairs(teamMates) do
            ...
            if fxTarget then
                TriggerClientEvent(
                    "deanix_delivery:Client:UpdateCurrentTaskCustomers",
                    mateSource,
                    newDestinations
                )
            end
...
RegisterNetEvent("deanix_delivery:Server:StartNewTaskDestination", function()
    local xPlayerSource = source
    local fxPlayer = Koci.Server.Functions.GetPlayer(xPlayerSource)
    if fxPlayer then
...
RegisterNetEvent("deanix_delivery:Server:OnCreatedTaskVehicle", function(netId, coords, plate)
    local src = source
    Koci.Server.Functions.OnCreatedTaskVehicle(src, netId, coords, plate)
end)

RegisterNetEvent("deanix_delivery:Server:OnCargoPutOnTaskVehicle", function()
    local src = source
...
RegisterNetEvent("deanix_delivery:Server:StopTask", function()
    local src = source
    Koci.Server.Functions.StopTask(src)
end)

RegisterNetEvent("deanix_delivery:Server:GetOnDuty", function()
    local src = source
    Koci.Server.Functions.GetOnDuty(src)
end)

RegisterNetEvent("deanix_delivery:Server:LeaveDuty", function()
    local src = source
    Koci.Server.Functions.LeaveDuty(src)
end)

Koci.Server.RegisterServerCallback("deanix_delivery:Server:OnTaskCompleted", function(source, cb)
    local xPlayerSource = source
    Koci.Server.Functions.OnTaskCompleted(xPlayerSource)
    cb(true)
end)

Koci.Server.RegisterServerCallback("deanix_delivery:Server:PlayerAcceptedInvitation", function(source, cb, inviting)
    ...
    if fxInvited and fxInviting then
        ...
        TriggerClientEvent("deanix_delivery:Client:TargetAcceptedInvitation",
            inviting,
            invited,
            invitedData
        )
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:SendInvitationToPlayer", function(source, cb, invited)
    local src = source
    if src == invited then
        cb({
            status = false,
    ...
    if fxTarget and fxPlayer then
        ...
        TriggerClientEvent("deanix_delivery:Client:NewInvitationArrived", invited, src, xPlayerCharacterName)
        cb({ status = true })
    else
        cb({ status = false, error = _t("game.player_not_found") })
    end
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:LoadPlayerData", function(source, cb)
    local src = source
    local xPlayer = Koci.Server.Framework.GetPlayerBySource(src)
    local PlayerData = {
        sourceId = src,
    ...
    if xPlayer then
        ...
        local _data = Koci.Server.ExecuteSQLQuery("SELECT * FROM `deanix_delivery_employees` WHERE user = ?",
            { identifier },
            "single"
        )
        if not _data or not next(_data) then
            Koci.Server.ExecuteSQLQuery(
                "INSERT INTO `deanix_delivery_employees` (user, profile, level, exp) VALUES (?, ?, ?, ?)",
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:StartTaskWithTeamMates", function(source, cb, taskId)
    local src = source
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:PickUpCargoFromCargoLoad", function(source, cb)
    local src = source
    if Koci.Server.Functions.CanLoadCargoInVehicle(src) then
        cb({ error = false })
        return
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:PickUpCargoFromTaskVehicle", function(source, cb)
    local src = source
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:PickUpCargoFromDestinationPed", function(source, cb)
    local src = source
    if Koci.Server.Functions.CanLoadCargoInVehicle(src) then
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:CanLoadCargoInVehicle", function(source, cb)
    local src = source
    if Koci.Server.Functions.CanLoadCargoInVehicle(src) then
        cb({ error = false })
        return
...
Koci.Server.RegisterServerCallback("deanix_delivery:Server:HandleTaskDestionCompleted", function(source, cb, prop)
    local src = source
