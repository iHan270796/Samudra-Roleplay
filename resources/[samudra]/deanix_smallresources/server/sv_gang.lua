...
lib.callback.register('gangmenu:server:registerStash', function(source, stashName)
    if not registeredStashes[stashName] then
        exports.ox_inventory:RegisterStash(stashName, 'Gang Boss - ' .. stashName, 25, 4000000, nil)
        registeredStashes[stashName] = true
    end
    return true
end)