...
RegisterNUICallback('home:get', function(_, cb)
    if Config.Housing == 'auto' then cb(nil) return end

    local properties = lib.callback.await('yseries:server:home:get', false)
...
RegisterNUICallback('home:get-key-holders-details', function(keyHolders, cb)
    local keyHoldersResult = lib.callback.await('yseries:server:home:get-key-holders-details', false, keyHolders)
...
RegisterNUICallback('home:remove-key-holder', function(data, cb)
    local keyHoldersResult = lib.callback.await('yseries:server:home:remove-key-holder', false, data)
...
RegisterNUICallback('home:add-key-holder', function(data, cb)
    local addedKeyholder = lib.callback.await('yseries:server:home:add-key-holder', false, data)
