...
function selectCategory(category) {
    ...
    $.post('https://deanix_vehicleshop/action', JSON.stringify({action: 'select-category'}));
    if (!category) {
...
$(document).on('click', '.vehicle', function(e) {
    ...
    if (model && price) {
        ...
        $.post('https://deanix_vehicleshop/action', JSON.stringify({
            action: 'select',
            model: model
        }));
    }
...
window.addEventListener('message', function(event) {
    switch(event.data.action) {
        case 'open':
            ...
            break;
        case 'close':
            $.post('https://deanix_vehicleshop/action', JSON.stringify({action: 'close'}));
            $("body").fadeOut(5)
            $('.right-bar').fadeOut(5)
...
window.addEventListener('keydown', function(event) {
    ...
    if (event.keyCode === 37) {
        $.post('https://deanix_vehicleshop/action', JSON.stringify({action: 'rotate-left'}));
    }
    if (event.keyCode === 39) {
        $.post('https://deanix_vehicleshop/action', JSON.stringify({action: 'rotate-right'}));
    }
}, false);

$(document).on('click', '.buy-button', function(e){
    $.post("https://deanix_vehicleshop/action", JSON.stringify({action: 'buy'}));
});

$(document).on('click', '.test-button', function(e){
    $.post("https://deanix_vehicleshop/action", JSON.stringify({action: 'test'}));
});