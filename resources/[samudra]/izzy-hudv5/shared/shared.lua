if not IsDuplicityVersion() then --client
    ...
    elseif framework == 'esx' then
        RegisterNetEvent('esx:playerLoaded', function()
            initialize()
            izzy.triggerCallback('izzy-hudv5:server:getName', function(val)
                name = val
            end)
    ...
    RegisterNetEvent("izzy-hudv5:client:setDisplay", function(state)
        current.initialized = state
        SendReactMessage('setDisplay', state)
    end)
    ...
    getStatus = function(statIndex)
        ...
        elseif statIndex == 'mic' then
            return cfg.voiceRanges[tostring(getVoiceState().range)] or 66
        ...
        elseif data.id == "job" then
            if framework == "qb" then
                if QBCore and QBCore.Functions.GetPlayerData() and QBCore.Functions.GetPlayerData().job then
                    data.value = ("%s - %s"):format(QBCore.Functions.GetPlayerData().job.label, QBCore.Functions.GetPlayerData().job.grade.name)
                end
            elseif framework == "esx" then
                if ESX and ESX.GetPlayerData() and ESX.GetPlayerData().job then
                    data.value = ("%s - %s"):format(ESX.GetPlayerData().job.label, ESX.GetPlayerData().job.grade_label)
                end
            end
        end
