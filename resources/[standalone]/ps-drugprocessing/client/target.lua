...
CreateThread(function()
    -- Walter
    local walterModel = `a_m_m_hillbilly_02`
    RequestModel(walterModel)
    while not HasModelLoaded(walterModel) do Wait(0) end
    local walter = CreatePed(0, walterModel, -1187.73, -445.27, 43.91 - 1.0, 289.45, false, true)
    FreezeEntityPosition(walter, true)
    SetEntityInvincible(walter, true)
    SetBlockingOfNonTemporaryEvents(walter, true)
    exports.ox_target:addLocalEntity(walter, {
        {
            name = 'walter_talk',
            event = 'ps-drugprocessing:EnterLab',
            type = 'client', -- tambahkan ini supaya event client kepanggil
            icon = 'fas fa-atom',
            label = Lang:t("target.talk_to_walter"),
            distance = 2.5
        }
    ...
    -- Draco
    local dracoModel = `a_m_m_mlcrisis_01`
    RequestModel(dracoModel)
    while not HasModelLoaded(dracoModel) do Wait(0) end
    local draco = CreatePed(0, dracoModel, 812.49, -2399.59, 23.66 - 1.0, 223.1, false, true)
    FreezeEntityPosition(draco, true)
    SetEntityInvincible(draco, true)
    SetBlockingOfNonTemporaryEvents(draco, true)
    exports.ox_target:addLocalEntity(draco, {
        {
            name = 'draco_talk',
            event = 'ps-drugprocessing:EnterCWarehouse',
            icon = 'fas fa-key',
    ...
    -- Charlotte
    local charlotteModel = `mp_f_weed_01`
    RequestModel(charlotteModel)
    while not HasModelLoaded(charlotteModel) do Wait(0) end
    local charlotte = CreatePed(0, charlotteModel, 102.07, 175.08, 104.59 - 1.0, 159.91, false, true)
    FreezeEntityPosition(charlotte, true)
    SetEntityInvincible(charlotte, true)
    SetBlockingOfNonTemporaryEvents(charlotte, true)
    exports.ox_target:addLocalEntity(charlotte, {
        {
            name = 'charlotte_talk',
            event = 'ps-drugprocessing:EnterWWarehouse',
            icon = 'fas fa-key',
...
CreateThread(function()
    ...
    exports["qb-target"]:AddBoxZone("cokeleafproc", vector3(1086.2, -3194.9, -38.99), 2.5, 1.4, {
        ...
        options = {
            {
                type = "client",
                event = "deanix_crafting:client:openprosescocain1",
    ...
    exports["qb-target"]:AddBoxZone("cokepowdercut", vector3(1092.89, -3195.78, -38.99), 7.65, 1.2, {
        ...
        options = {
            {
                type = "client",
                event = "deanix_crafting:client:openprosescocain2",
    ...
    exports["qb-target"]:AddBoxZone("weedproces", vector3(1038.37, -3206.06, -38.17), 2.6, 1.0, {
        ...
        options = {
            {
                type = "client",
                event = "deanix_crafting:client:openweedproses",
                icon = "fas fa-envira",
    ...
    exports["qb-target"]:AddBoxZone("heroinproces", vector3(1391.41, 3605.55, 39.07), 2.5, 2.5, {
        name = "heroinproces",
        heading = 223.98,
        debugPoly = false,
        minZ = 37.21,
        ...
        options = {
            {
                type = "client",
                event = "deanix_crafting:client:openheroinproses",
                icon = "fas fa-envira",
