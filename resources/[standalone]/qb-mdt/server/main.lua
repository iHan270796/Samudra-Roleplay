...
local MugShotWebhook = 'https://discord.com/api/webhooks/1381919936040927232/04FrTQE_VD80MrLFi_I5Jh4C13HH3-AkXpd-BEn_CTu1e3vC0tKRer8xGy52P-PDARCG'

-- Clock-in notifications for duty. Add a Discord webhook.
-- Command /mdtleaderboard, will display top players per clock-in hours.
local ClockinWebhook = 'https://discord.com/api/webhooks/1381919936040927232/04FrTQE_VD80MrLFi_I5Jh4C13HH3-AkXpd-BEn_CTu1e3vC0tKRer8xGy52P-PDARCG'

-- Incident and Incident editting. Add a Discord webhook.
-- Incident Author, Title, and Report will display in webhook post.
local IncidentWebhook = 'https://discord.com/api/webhooks/1381919936040927232/04FrTQE_VD80MrLFi_I5Jh4C13HH3-AkXpd-BEn_CTu1e3vC0tKRer8xGy52P-PDARCG'
--------------------------------

QBCore.Functions.CreateCallback('qb-mdt:server:MugShotWebhook', function(source, cb)
...
RegisterServerEvent("qb-mdt:dispatchStatus", function(bool)
...
QBCore.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    ...
    sendToDiscord(16753920, "MDT Leaderboard", leaderboard_message, "qb-mdt | Made by Project Sloth")
    TriggerClientEvent('QBCore:Notify', source, "MDT leaderboard sent to Discord!", 'success')
end)

RegisterNetEvent("qb-mdt:server:ClockSystem", function()
    local src = source
    local PlayerData = GetPlayerData(src)
    local time = os.date("%Y-%m-%d %H:%M:%S")
    local firstName = PlayerData.charinfo.firstname:sub(1,1):upper()..PlayerData.charinfo.firstname:sub(2)
...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	if GetResourceState('qb-dispatch') == 'started' then
		calls = exports['qb-dispatch']:GetDispatchCalls()
	end
...
QBCore.Functions.CreateCallback('qb-mdt:getDispatchCalls', function(source, cb)
    local calls = exports['qb-dispatch']:GetDispatchCalls()
    cb(calls)
end)
                        ...
                        MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = infoResult }, function(result)
                            if result and #result > 0 then
                                ...
                                for i=1, #associated do
                                    ...
                                    sendIncidentToDiscord(3989503, "MDT Incident Report", message, "qb-mdt | Made by Project Sloth", associatedData)                                
                                end
                            else
                ...
                MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = id }, function(result)
                    if result and #result > 0 then
                        ...
                        for i=1, #associated do
                            ...
                            sendIncidentToDiscord(16711680, "MDT Incident Report has been Updated", message, "qb-mdt | Made by Project Sloth", associatedData)
                        end
                    else
...
RegisterNetEvent('mdt:server:registerweapon', function(serial, imageurl, notes, owner, weapClass, weapModel) 
    exports['qb-mdt']:CreateWeaponInfo(serial, imageurl, notes, owner, weapClass, weapModel)
end)
