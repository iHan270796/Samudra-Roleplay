...
RegisterNetEvent('dw-bossmenu:server:UpdateEmployee', function(citizenid, jobName, grade)
    ...
    -- convert grade ke number
    grade = tonumber(grade)
    if not grade or not QBCore.Shared.Jobs[jobName] or not QBCore.Shared.Jobs[jobName].grades[grade] then
        ...
        if targetPlayer then
            ...
            if targetCurrentJob == jobName then
                ...
                TriggerClientEvent('QBCore:Notify', targetPlayer.PlayerData.source,
                    "Your rank has been updated to " .. QBCore.Shared.Jobs[jobName].grades[grade].name, "success")
                ...
                TriggerClientEvent('QBCore:Notify', targetPlayer.PlayerData.source,
                    "Your job has been changed to " .. QBCore.Shared.Jobs[jobName].label, "success")
                TriggerClientEvent('QBCore:Notify', src, "Employee successfully transferred to this job", "success")
            end
        else
            -- Offline player update (biarin sama, tapi pakai grade number)
        end
    end)
end)
...
RegisterNetEvent('dw-bossmenu:server:RemoveEmployee', function(citizenid, jobName)
    ...
    if not Player or Player.PlayerData.job.name ~= jobName then
        return TriggerClientEvent('QBCore:Notify', src, "You don't have permission for this action", "error")
    end
    ...
    MySQL.Async.fetchScalar('SELECT job FROM players WHERE citizenid = @citizenid', {
        ...
        if not result then
            return TriggerClientEvent('QBCore:Notify', src, "Player not found", "error")
        end

        local currentJob = json.decode(result)
        if not currentJob or currentJob.name ~= jobName then
            return TriggerClientEvent('QBCore:Notify', src, "This player doesn't work for this job", "error")
        end
        ...
        if targetPlayer then
            ...
            TriggerEvent('qb-log:server:CreateLog', 'jobmanagement', 'Employee Fired', 'red',
                string.format('%s (%s) fired %s (%s) from job %s',
                GetPlayerName(src), Player.PlayerData.citizenid,
                GetPlayerName(targetPlayer.PlayerData.source), citizenid, jobName))
        else
            local jobInfo = {
                name = "unemployed",
                label = QBCore.Shared.Jobs["unemployed"].label,
                payment = QBCore.Shared.Jobs["unemployed"].grades[0].payment,
                onduty = true,
                grade = {
                    name = QBCore.Shared.Jobs["unemployed"].grades[0].name,
                    level = 0
                },
                isboss = false
            }
            ...
            MySQL.Async.execute('UPDATE players SET job = @job WHERE citizenid = @citizenid', {
                ...
                if rowsChanged > 0 then
                    TriggerClientEvent('QBCore:Notify', src, "Employee removed successfully", "success")
                    TriggerEvent('qb-log:server:CreateLog', 'jobmanagement', 'Offline Employee Fired', 'red',
                        string.format('%s (%s) fired %s from job %s',
...
QBCore.Functions.CreateCallback('dw-bossmenu:server:CheckApplicationStatus', function(source, cb, jobName)
    ...
    MySQL.Async.fetchAll('SELECT status, date_reviewed FROM job_applications WHERE citizenid = @citizenid AND job = @jobName ORDER BY date_submitted DESC LIMIT 1', {
        ...
        elseif lastApplication.status == "rejected" then
            ...
            local cooldownTime = 3 * 24 * 60 * 60 -- 3 days in seconds
...
QBCore.Functions.CreateCallback('dw-bossmenu:server:HireNewEmployee', function(source, cb, targetId, jobName, grade)
    ...
    if not Player then
        return cb({ success = false, message = "Error: Manager not found" })
    end

    if not Target then
        return cb({ success = false, message = "Player with ID " .. targetId .. " not found" })
    end

    if Player.PlayerData.job.name ~= jobName then
        return cb({ success = false, message = "You don't have permission for this job" })
    end

    if not Player.PlayerData.job.isboss then
        return cb({ success = false, message = "You don't have hiring permissions" })
    end

    -- pastikan grade integer
    grade = tonumber(grade)

    if not job or not job.grades[grade] then
        return cb({ success = false, message = "Invalid job grade" })
    end
    ...
    TriggerClientEvent('QBCore:Notify', Target.PlayerData.source,
        "You've been hired as " .. job.grades[grade].label .. " at " .. job.label, "success")
    TriggerClientEvent('QBCore:Notify', src,
        "You hired " .. GetPlayerName(Target.PlayerData.source) .. " as " .. job.grades[grade].label, "success")

    -- Logging
    TriggerEvent('qb-log:server:CreateLog', 'jobmanagement', 'Employee Hire', 'green',
        string.format('%s (%s) hired %s (%s) for job %s at rank %s',
        GetPlayerName(src), Player.PlayerData.citizenid,
        GetPlayerName(Target.PlayerData.source), Target.PlayerData.citizenid,
        jobName, grade))

    return cb({ success = true, message = "Employee hired successfully" })
end)
