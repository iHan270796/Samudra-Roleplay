    ...
    if inv == 'player' or inv == 'showbpk' and data ~= cache.serverId then
		...
		if inv == 'showbpk' then
			...
			if Config["Backpack"].requireItem then
				check = lib.callback.await('raihan-inventory:GetBackpackItem', 200, data)
			end
...
RegisterNetEvent('raihan-inventory:setArmor', function(value)
...
--]]
	...
	CreateThread(function()
		...
		if hasBackpack and backpackState then
			...
			TriggerServerEvent('raihan-inventory:SyncBackpackClothes', backpackState.id)
		end
	end)
	...
	client.interval = SetInterval(function()
		...
		if client.parachute and GetPedParachuteState(playerPed) ~= -1 then
			Utils.DeleteEntity(client.parachute[1])
			client.parachute = false
			TriggerServerEvent('raihan-inventory:RemoveParachute')
		end
	...
	TriggerEvent('raihan-inventory:Client:ApplyArmorFromRejoinn')
...
lib.callback.register('ox_inventory:startCrafting', function(id, recipe) 
	...
	return lib.progressBar({ 
		label = locale('crafting_item', recipe.metadata?.label or 
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	if data.toSlot == 2 and data.item == "armour" then
		TriggerServerEvent("raihan-inventory:Server:ReApplyPlates")
	end
