...
RegisterCommand('+radiotalk', function()
    ...
    if not radioPressed then
        if radioChannel > 0 then
            ...
            local propCoords = vector3(0.0750, 0.0230, -0.0230)
            local propRot = vector3(-90.0, 0.0, -59.9999)

            RequestAnimDict(dict)
            while not HasAnimDictLoaded(dict) do Wait(10) end
            TaskPlayAnim(ped, dict, anim, 8.0, 2.0, -1, 49, 0.0, false, false, false)
            ...
            AttachEntityToEntity(prop, ped, GetPedBoneIndex(ped, boneIndex),
