...
function OpenTVMenu()
    ...
    -- Submenu untuk channels
    local channelOptions = {}
    for i, channel in ipairs(ChannelList.list) do
        channelOptions[#channelOptions+1] = {
            title = channel.name,
            description = 'Switch to this channel',
            onSelect = function()
                SetChannel(channel.index)
                OpenTVMenu() -- buka lagi menu utama setelah pilih
            end
        }
    end
    ...
    lib.registerContext({
        ...
        options = {
            {
                title = 'Videos',
                description = 'Play a video or stream on the screen.',
                onSelect = function()
                    VideoMenu()
                end
            },
            {
                title = 'Web Browser',
                description = 'Access the web via your TV.',
                onSelect = function()
                    WebBrowserMenu()
                end
            },
            {
                title = 'TV Channels',
                description = 'Live TV Channels in San Andreas!',
                menu = 'ptelevision-channels'
            },
            {
                title = 'Interact With Screen',
                description = 'Allows you to control on-screen elements.',
                onSelect = function()
                    SetInteractScreen(true)
                end
            },
            {
                title = 'Set Volume',
                description = 'Sets your TV\'s volume (For yourself).',
                onSelect = function()
                    VolumeMenu()
                end
            },
    ...
    lib.registerContext({
        id = 'ptelevision-channels',
        title = 'TV Channels',
        menu = 'ptelevision-menu',
        options = channelOptions
    })

    lib.showContext('ptelevision-menu')
end
