...
Citizen.CreateThread(function()
    ...
    -- cek kalau pakai target system
    if Config.UseTarget then
        for k, v in pairs(Config.VehicleShops) do
            Config.Target(k, v)
        end
    else
        -- fallback ke 3D text kalau ga pakai target
        while true do
            local sleep = 2000
            ...
            for k, v in pairs(Config.VehicleShops) do
                if v.Owner ~= "" then
                    ...
                    if dist < 2.0 then
                        sleep = 3
                        DrawText3D("Tekan [E] untuk buka dealer", v.coords)
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("deanix_vehicleshop:open", k)
                        end
                    end
                end
            end
...
function GeneratePlate()
	...
	while true do
		...
		generatedPlate = ((Config.PlateCustomPrefix and Config.PlateCustomPrefix or string.upper(GetRandomLetter(Config.PlateLetters))).. ' '..GetRandomNumber(Config.PlateNumbers))
		if Config.Core == "ESX" then
            ESX.TriggerServerCallback('deanix_vehicleshop:isPlateTaken', function(isPlateTaken)
                if not isPlateTaken then
                    isNotTaken = true
                end
            end, generatedPlate)
        elseif Config.Core == "QB-Core" then
            QBCore.Functions.TriggerCallback('deanix_vehicleshop:isPlateTaken', function(isPlateTaken)
                if not isPlateTaken then
                    isNotTaken = true
                end
            end, generatedPlate)
...
RegisterNUICallback("action", function(data)
    if data.action == "close" then
        ...
        if Config.UseRoutingBucketsInShowRoom then
            TriggerServerEvent("deanix_vehicleshop:setRoutingBucket", true)
        end
        Config.Hud:Enable()
    ...
    elseif data.action == "buy" then
        if Config.Core == "ESX" then
            ESX.TriggerServerCallback('deanix_vehicleshop:buyVehicle', function(callback) 
                if callback then
                    if Config.UseRoutingBucketsInShowRoom then
                        TriggerServerEvent("deanix_vehicleshop:setRoutingBucket", true)
                    ...
                    Citizen.CreateThread(function() 
                        ...
                        ESX.Game.SpawnVehicle(currentVehicle, vec(currentVehicleshop.buyCoords.x, currentVehicleshop.buyCoords.y, currentVehicleshop.buyCoords.z), currentVehicleshop.buyCoords.w, function(vehicle)
                            ...
                            TriggerServerEvent('deanix_vehicleshop:addOwner', vehicleProps, currentVehicleshop.vehicles[currentVehicle], currentVehicleshop.type)
        ...
        elseif Config.Core == "QB-Core" then
            QBCore.Functions.TriggerCallback('deanix_vehicleshop:buyVehicle', function(callback)
                if callback then
                    if Config.UseRoutingBucketsInShowRoom then
                        TriggerServerEvent("deanix_vehicleshop:setRoutingBucket", true)
                        while isInRoutingBucket do
                    ...
                    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                        ...
                        Config.GiveKeysBuy(vehicle, newPlate, currentVehicleshop.vehicles[currentVehicle].model)
                        if DoesCamExist(cam) then
                            DestroyCam(cam, true)
                            RenderScriptCams(false, true, 1)
                            cam = nil
    ...
    elseif data.action == "test" then
        ...
        if currentVehicleshop.testDrivePrice and currentVehicleshop.testDrivePrice > 0 then
            canTestDrive = nil
            if Config.Core == "ESX" then
                ESX.TriggerServerCallback('deanix_vehicleshop:buyTestDrive', function(callback) 
                    canTestDrive = callback
                end, currentVehicleshop.testDrivePrice)
            elseif Config.Core == "QB-Core" then
                QBCore.Functions.TriggerCallback('deanix_vehicleshop:buyTestDrive', function(callback)
                    canTestDrive = callback
                end, currentVehicleshop.testDrivePrice)
            end
        ...
        if canTestDrive then
            ...
            if vehicle and DoesEntityExist(vehicle) then
                if Config.UseRoutingBucketsOnTestDrive then
                    TriggerServerEvent("deanix_vehicleshop:setRoutingBucket")
                else
                    TriggerServerEvent("deanix_vehicleshop:setRoutingBucket", true)
                end
...
RegisterNetEvent("deanix_vehicleshop:settedRoutingBucket", function(toggle)
    isInRoutingBucket = toggle
end)

RegisterNetEvent("deanix_vehicleshop:open", function(id)
    local v = Config.VehicleShops[id]
    openVehicleshop(v)
end)

RegisterNetEvent('deanix_vehicleshop:notification', function(message, time, type)
