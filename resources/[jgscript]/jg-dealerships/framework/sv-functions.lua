...
function Framework.Server.IsAdmin(src)
  ...
  if _G.JG_ADMIN_BYPASS then
    print("[JG-Dealerships] WARNING: Admin bypass is enabled!")
    return true
  end
  
  -- First check ACE permissions (works for all frameworks)
  if IsPlayerAceAllowed(tostring(src), "command") or IsPlayerAceAllowed(tostring(src), "admin") then
    return true
  end
  
  -- Check framework-specific permissions
  if Config.Framework == "Qbox" then
    -- Try multiple Qbox permission methods
    local success, result = pcall(function()
      -- Method 1: Try GetPermission if it exists
      if exports.qbx_core and exports.qbx_core.GetPermission then
        local perms = exports.qbx_core:GetPermission(src)
        if perms and (perms == "admin" or perms == "god") then
          return true
        end
      end
      
      -- Method 2: Try checking player group
  ...
  -- Final fallback - check if player is server owner (source 0 or console)
  return false
end
