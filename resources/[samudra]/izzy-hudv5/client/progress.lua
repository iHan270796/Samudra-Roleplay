-- Register progress bar export
function registerProgressExport(name, cb)
    AddEventHandler(string.format('__cfx_export_progressbar_%s', name), function(setCB)
        setCB(cb)
    end)
...
-- Control mappings for disabling player actions
local controls = {
...
local function disableControls()
    CreateThread(function()
        while isDoingAction do
            if Action.controlDisables then
...
RegisterNetEvent('progressbar:client:cancel', function()
    ActionCleanup()
    SendReactMessage('cancelProgress')
end)

-- NUI Callback
RegisterNUICallback('onProgressEnd', function(data, cb)
    ActionCleanup()
    cb({ 'ok' })
end)
...
registerProgressExport('Progress', Progress)

local function ProgressWithStartEvent(action, start, finish)
    StartProgress(action, start, nil, finish)
end
registerProgressExport('ProgressWithStartEvent', ProgressWithStartEvent)

local function ProgressWithTickEvent(action, tick, finish)
    StartProgress(action, nil, tick, finish)
end
registerProgressExport('ProgressWithTickEvent', ProgressWithTickEvent)

local function ProgressWithStartAndTick(action, start, tick, finish)
    StartProgress(action, start, tick, finish)
end
registerProgressExport('ProgressWithStartAndTick', ProgressWithStartAndTick)

local function isDoingSomething()
    return isDoingAction
end
registerProgressExport('isDoingSomething', isDoingSomething)

izzy.registerKeyMap({
    command = cfg.cancelProgress.command,
    key = cfg.cancelProgress.key,
    description = cfg.cancelProgress.description,
}, function()
    TriggerEvent('progressbar:client:cancel')
    wasCancelled = true
end)