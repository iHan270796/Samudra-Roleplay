...
function Utils.Functions.AddWaisExtraDialog(entity, point)
    if Koci.Utils.hasResource("wais-npcdialog") then
        exports["wais-npcdialog"]:addExtraDialog({
            entity = entity,
            label = "deanix_delivery:toggleDuty",
            coords = vector4(
            ...
            options = {
                option1 = {
                    ...
                    trigger = "deanix_delivery:Client:ToggleDeliveryJobDuty",
                    eventType = "client",
                    selected = false,
                },
                option2 = {
...
function Utils.Functions.DrawTextUI(text, pos)
    if Config.TextUIType == "ox_textui" then
        lib.hideTextUI()
        lib.showTextUI(text, {
            position = pos or 'left-center',
        })
