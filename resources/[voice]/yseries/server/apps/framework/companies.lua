...
function IsBlocked(job, phoneNumber)
    local isBlocked = MySQL.scalar.await([[
        SELECT COUNT(*) AS totalMessages
        FROM yphone_blocked_numbers
        WHERE phone_imei LIKE @phoneImei AND blocked_number = @blockedNumber
    ]], { ["@phoneImei"] = '%:' .. job, ["@blockedNumber"] = phoneNumber })


    return isBlocked > 0
end

local function blockCaller(source, number)
    ...
    if IsBlocked(company.job, number) then
        return true
    end

    -- phone_imei is required to be unique, so we use the company name + a random uuid
    -- we can later search by the company name (e.g. {uuid}:{company_name})
    local uuid = ('%s:%s'):format(GenerateUid(), company.job)

    if company and company.management.block then
        -- Add the number to blocked numbers table
        MySQL.insert.await('INSERT INTO yphone_blocked_numbers (phone_imei, blocked_number) VALUES (?, ?)', {
            uuid,
            number
        })
...
local function unblockCaller(job, number)
    local company = nil
    for i = 1, #Config.Companies.Services do
        local service = Config.Companies.Services[i]
        if service.job == job then
    ...
    local result = MySQL.scalar.await('DELETE FROM yphone_blocked_numbers WHERE phone_imei LIKE @phoneImei AND blocked_number = @blockedNumber', {
        ["@phoneImei"] = '%:' .. company.job,
...
lib.callback.register('yseries:server:companies:block-caller', function(source, number)
    if not number or number == '' then
        return false
    end

    return blockCaller(source, number)
end)
...
-- Check if the caller is blocked by the company
lib.callback.register('yseries:server:companies:is-blocked', function(_, job, phoneNumber)
    return IsBlocked(job, phoneNumber)
end)

lib.callback.register('yseries:server:companies:unblock-caller', function(_, job, phoneNumber)
    return unblockCaller(job, phoneNumber)
end)

lib.callback.register('yseries:server:companies:call-employee', function(_, phoneNumber, targetSource, callerNumber, job)
    debugPrint('Calling employee.. ', phoneNumber, targetSource, callerNumber, job)

    local isBlocked = IsBlocked(job, phoneNumber)

    if isBlocked then
        TriggerClientEvent('yseries:client:companies:cancel-call', targetSource)
...
lib.callback.register('yseries:server:companies:get-blocked-numbers', function(_, job)
    local blockedNumbers = MySQL.query.await([[
        SELECT * FROM yphone_blocked_numbers WHERE phone_imei LIKE @phoneImei
    ]], { ["@phoneImei"] = '%:' .. job })
