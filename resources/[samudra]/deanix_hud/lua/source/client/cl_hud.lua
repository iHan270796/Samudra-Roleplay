...
local function loadMap()
    CreateThread(function()
        ...
        Wait(200)
        DisplayRadar(not forceHideMinimap)
    end)
end
...
AddEventHandler(Bridge.Events.ESXOnly.StatusOnTick, function(data)
    for i = 1, #data do
        ...
        ESX.TriggerServerCallback("deanix_hud:RetrieveStress", function(stress)
            stats.stress = stress
        end)
        Wait(1000)
        TriggerEvent("hud:client:UpdateNeeds", stats.hunger, stats.thirst)
        TriggerEvent("hud:client:UpdateStress", stats.stress)
    end
end)
