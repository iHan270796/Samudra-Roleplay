local Config = require 'c_boombox'
...
RegisterNetEvent('wasabi_boombox:removeBoombox', function()
    local src = source
    exports.ox_inventory:RemoveItem(src, Config.BoomboxItem, 1)
end)
...
if Framework == "ESX" then
    RegisterServerEvent('wasabi_boombox:save', function(name, link)
        local xPlayer = ESX.GetPlayerFromId(source)
...
if Framework == "ESX" then
    RegisterServerEvent('wasabi_boombox:deleteSong', function(data)
        local xPlayer = ESX.GetPlayerFromId(source)
...
if Framework == "ESX" then
    RegisterNetEvent("wasabi_boombox:DiscordKnows", function(link)
        local xPlayer = ESX.GetPlayerFromId(source)
...
elseif Framework == "qb" then
    ...
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then
        print("Boombox Error: Player not found for source " .. tostring(source))
        return
    end
    local CitizenId = Player.PlayerData.citizenid
