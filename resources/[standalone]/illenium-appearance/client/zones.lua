...
RegisterNetEvent("illenium-appearance:client:OpenClothingRoom", function()
    ...
    if currentZone and currentZone.index then
        index = currentZone.index
    elseif #Config.ClothingRooms == 1 then
        index = 1
    else
        lib.notify({
            title = "Illenium Appearance",
            description = "Tidak dapat menemukan clothing room yang aktif!",
            type = "error"
        })
        return
    end
    ...
    local clothingRoom = Config.ClothingRooms[index]
    if not clothingRoom then
        lib.notify({
            title = "Illenium Appearance",
            description = "Clothing room index " .. tostring(index) .. " tidak ditemukan di config!",
            type = "error"
        })
        return
    end
