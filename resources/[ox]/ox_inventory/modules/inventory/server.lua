...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
    ...
    if not toSlot then
        ...
        for i = #UtilsSlot + 1, inv.slots do
...
local function dropItem(source, fromInventory, fromData, data, playerInventory)
    ...
    if slot == fromInventory.weapon then
    ...
    if server.syncInventory and fromInventory.type == 'player' then server.syncInventory(fromInventory) end
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
    ...
    local toInventory = (data.toType == 'player' and playerInventory) or
        (data.toType == 'container' and Inventory(playerInventory.other)) or Inventory(playerInventory.open)

    local fromInventory = (data.fromType == 'player' and playerInventory) or
        (data.fromType == 'container' and Inventory(playerInventory.other)) or Inventory(playerInventory.open)
    ...
    if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
        ...
        if data.toType == 'newdrop' then
            return dropItem(source, fromInventory, fromData, data, playerInventory)
        end
