if Config.Framework == "qb" or Config.Framework == "qbx" then
    RegisterServerEvent("samudra_hud:GainStress", function(amount)
        if Config.Settings.EnableStress then
            ...
            TriggerClientEvent("samudra_hud:RecieveStress", src, NewStress)
            Bridge.Notify("Stress Gained!", "error", 4000)
        end
    end)
    
    RegisterServerEvent("samudra_hud:RelieveStress", function(amount)
        if Config.Settings.EnableStress then
            local src = source
            local PlayerMetadata = Bridge.GetPlayerMetadata(src)
            local NewStress
...
ESX.RegisterServerCallback("samudra_hud:RetrieveStress", function(src, cb)
...
RegisterNetEvent('samudra_hud:GainStress', function(amount)
    local src = source
...
RegisterNetEvent('samudra_hud:RelieveStress', function(amount)
